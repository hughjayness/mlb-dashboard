...
async function maybeFetchTopPropForEvent({ eventId, apiKey, lineupContext, modelOutputs, viewMode, priorityRank }) {
  ...
  const rank = typeof priorityRank === "number" ? priorityRank : 999;
  const alwaysFetchForMode = viewMode === "props";
  const shouldFetch =
    alwaysFetchForMode ||
    rank < 5 ||
  ...
  if (!shouldFetch) {
    return {
      topProp: null,
      status: "Optional prop was not evaluated for this lower-priority game in the current load.",
      lineupMode: lineupContext.lineupMode,
      lineupSource: lineupContext.lineupSource
    };
  }
