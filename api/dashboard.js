...
async function fetchFeaturedOdds(apiKey) {
  const url =
    ...
    `&markets=${encodeURIComponent(FEATURED_MARKETS.join(","))}` +
...
function buildGameResponse(args) {
  ...
  return {
    ...
    firstFiveRunLine: `${event.away_team} ${formatSignedPoint(context.f5RunLineData.awayPoint)} (${hasValue(context.f5RunLineData.awayPrice) ? (context.f5RunLineData.awayPrice > 0 ? "+" : "") + context.f5RunLineData.awayPrice : "—"}) | ${event.home_team} ${formatSignedPoint(context.f5RunLineData.homePoint)} (${hasValue(context.f5RunLineData.homePrice) ? (context.f5RunLineData.homePrice > 0 ? "+" : "") + context.f5RunLineData.homePrice : "—"})`,
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const events = safeArray(featuredOdds)
      .filter(e => e?.id && e?.away_team && e?.home_team)
