// api/dashboard.js
// Full replacement V8: richer real-world analysis layer + odds ingestion diagnostics + same-game scenario arbitration.
...
function buildPropCandidates(propOdds, ctx) {
  ...
  for (const row of map.values()) {
    ...
    for (const side of ["over", "under"]) {
      ...
      const fairProb = clamp(implied + tilt * (side === "over" ? 0.28 : 0.24), 0.05, 0.95);
      out.push({
        ...
        modelProb: round2(fairProb), reasons: [`${label} ${side} evaluated across all available prop books.`, `Pitcher/team/environment model tilt: ${round2(tilt)}`]
      });
    }
  }
  return out;
  ...
  if (!playable.length) {
    return arr.map(g => addConfidenceFields({
      ...
      confidenceForceDebug: {
        forceRan: true,
        calibrationMode: "v8_real_world_analysis_phase1",
        viewMode,
        playableRows: 0,
