module.exports = async function handler(req, res) {
  ...
  const requestedModeRaw = (req && req.query && (req.query.view || req.query.mode)) ? String(req.query.view || req.query.mode) : "full";
  const requestedMode = normalizeViewMode(requestedModeRaw);
  ...
  try {
    ...
    const datesNeeded = Array.from(new Set(
      limitedGames
        .map(g => toDateEt(g.commence_time))
        .filter(Boolean)
    ));
    ...
    const baseGameContexts = limitedGames.map((game, index) => {
      ...
      const gameContext = findGameContext(scheduleContext, awayTeam, homeTeam, pitcherStatsMap);
    ...
    const f5EligibleKeys = new Set(
      requestedMode === "f5"
        ? baseGameContexts
            .slice()
            .sort((a, b) => rankF5Priority(b) - rankF5Priority(a))
            .slice(0, 4)
            .map(ctx => String(ctx.game.id))
        : []
    );

    const propEligibleKeys = new Set(
      requestedMode === "props"
        ? baseGameContexts
            .slice()
            .sort((a, b) => rankPropPriority(b) - rankPropPriority(a))
            .slice(0, 4)
            .map(ctx => String(ctx.game.id))
        : []
    );
    ...
    const games = await Promise.all(
      baseGameContexts.map(async (ctx) => {
        const { game, index, homeTeam, awayTeam, moneylineData, spreadData, totalData, lineupContext, gameContext, parkContext, componentScores, modelOutputs } = ctx;

        let firstFiveData = emptyFirstFiveData();
        if (requestedMode === "f5" && f5EligibleKeys.has(String(game.id))) {
          firstFiveData = await safeFetchFirstFiveMarketsForEvent({
            eventId: game.id,
            apiKey,
            homeTeam,
            awayTeam
        ...
        let propResult = skippedPropResult(lineupContext, "Props are loaded only in Props mode.");
        if (requestedMode === "props") {
          if (propEligibleKeys.has(String(game.id))) {
            propResult = await safeMaybeFetchTopPropForEvent({
              eventId: game.id,
              apiKey,
              lineupContext,
              modelOutputs
            ...
            propResult = skippedPropResult(lineupContext, "Prop lookup skipped because this game did not qualify for the smaller Props-mode candidate set.");
          }
        }
...
function normalizeViewMode(mode) {
  ...
  if (raw === "f5" || raw === "first5" || raw === "first-five") return "f5";
  if (raw === "props" || raw === "prop") return "props";
  return "full";
}

function emptyFirstFiveData() {
  return {
    moneyline: { display: "N/A", awayPrice: null, homePrice: null, awayProb: null, homeProb: null },
    spread: { display: "N/A", awayPoint: null, homePoint: null, awayPrice: null, homePrice: null },
    total: { display: "N/A", point: null, overPrice: null, underPrice: null, overProb: null, underProb: null }
  };
}

function skippedPropResult(lineupContext, status) {
  return {
    topProp: null,
    status: status || "Prop lookup skipped.",
    lineupMode: lineupContext && lineupContext.lineupMode ? lineupContext.lineupMode : "",
    lineupSource: lineupContext && lineupContext.lineupSource ? lineupContext.lineupSource : ""
  };
}

function rankF5Priority(ctx) {
  const componentScores = ctx && ctx.componentScores ? ctx.componentScores : {};
  const modelOutputs = ctx && ctx.modelOutputs ? ctx.modelOutputs : {};
  const starterEdge = Math.abs(toNumber(componentScores.side && componentScores.side.startingPitcher) || 0);
  const bullpenEdge = Math.abs(toNumber(componentScores.side && componentScores.side.bullpen) || 0);
  const sideEdge = Math.max(
    Math.abs(toNumber(modelOutputs.homeEdgePct) || 0),
    Math.abs(toNumber(modelOutputs.awayEdgePct) || 0)
  );
  const totalEdge = Math.max(
    Math.abs(toNumber(modelOutputs.overEdgePct) || 0),
    Math.abs(toNumber(modelOutputs.underEdgePct) || 0)
  );
  return starterEdge * 2.2 + Math.max(0, starterEdge - bullpenEdge) * 1.8 + sideEdge * 0.7 + totalEdge * 0.35;
}

function rankPropPriority(ctx) {
  const lineupContext = ctx && ctx.lineupContext ? ctx.lineupContext : {};
  const modelOutputs = ctx && ctx.modelOutputs ? ctx.modelOutputs : {};
  const starterEdge = Math.abs(toNumber(ctx && ctx.componentScores && ctx.componentScores.side && ctx.componentScores.side.startingPitcher) || 0);
  ...
  return (lineupContext.officialLineupAvailable ? 6 : 0) + gameEdge + starterEdge * 0.5;
}

async function safeFetchFirstFiveMarketsForEvent(args) {
  try {
    return await fetchFirstFiveMarketsForEvent(args);
  } catch (error) {
    if (error && String(error.message || error).includes("EXCEEDED_FREQ_LIMIT")) return emptyFirstFiveData();
    return emptyFirstFiveData();
  }
}

async function safeMaybeFetchTopPropForEvent(args) {
  try {
    return await maybeFetchTopPropForEvent(args);
  } catch (error) {
    const lineupContext = args && args.lineupContext ? args.lineupContext : {};
    return skippedPropResult(lineupContext, "Prop lookup skipped after an odds-provider rate-limit response.");
  }
}
...
function findGameContext(scheduleJson, awayTeam, homeTeam, pitcherStatsMap) {
  const out = {
  ...
  for (const date of dates) {
    ...
    for (const game of games) {
      ...
      const awayProbable = game && game.teams && game.teams.away && game.teams.away.probablePitcher;
      const homeProbable = game && game.teams && game.teams.home && game.teams.home.probablePitcher;

      out.awayProbablePitcher = mergeProbablePitcher(awayProbable, pitcherStatsMap);
      out.homeProbablePitcher = mergeProbablePitcher(homeProbable, pitcherStatsMap);
      return out;
    }
  }
...
function buildRecommendation({
  ...
  const allowFullGame = normalizedViewMode === "full";
  const allowF5 = normalizedViewMode === "f5";
  const allowProps = normalizedViewMode === "props";
  ...
  if (allowF5 && typeof firstFiveModelOutputs.homeEdgePct === "number" && shouldIncludeFirstFiveSideCandidate("home", firstFiveLogic, firstFiveModelOutputs, firstFiveData, componentScores)) {
    gameCandidates.push({
  ...
  if (allowF5 && typeof firstFiveModelOutputs.awayEdgePct === "number" && shouldIncludeFirstFiveSideCandidate("away", firstFiveLogic, firstFiveModelOutputs, firstFiveData, componentScores)) {
    gameCandidates.push({
  ...
  if (allowF5 && typeof firstFiveModelOutputs.overEdgePct === "number" && shouldIncludeFirstFiveTotalCandidate(firstFiveLogic, firstFiveModelOutputs, firstFiveData, componentScores)) {
    gameCandidates.push({
  ...
  if (allowF5 && typeof firstFiveModelOutputs.underEdgePct === "number" && shouldIncludeFirstFiveTotalCandidate(firstFiveLogic, firstFiveModelOutputs, firstFiveData, componentScores)) {
    gameCandidates.push({
  ...
  if (allowF5 && firstFiveData && firstFiveData.spread && firstFiveData.spread.homePoint !== null && firstFiveData.spread.awayPoint !== null) {
