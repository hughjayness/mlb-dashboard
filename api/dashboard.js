module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      notes: [
        "Official lineups are now checked from MLB starting-lineups page first.",
...
function parseMLBStartingLineups(html) {
  ...
  // Pull visible text blocks. This parser is intentionally simple and resilient.
  const cleaned = html
    .replace(/\r/g, "")
    .replace(/&nbsp;/g, " ")
    .replace(/&#x27;/g, "'")
  ...
  for (let i = 0; i < lines.length; i++) {
    if (lines[i].endsWith(" @") && i + 1 < lines.length) {
      // fallback path, not usually used
      continue;
    }

    // Match pattern around "Away @ Home"
    if (lines[i] === "@") continue;
    ...
    if (i + 2 < lines.length && lines[i + 1] === "@") {
      ...
      const block = lines.slice(i, Math.min(i + 120, lines.length));
...
function extractNinePlayers(block, startIndex) {
  ...
  for (let i = startIndex; i < block.length && players.length < 9; i++) {
    ...
    const m = line.match(/^\d+\.\s+(.+?)\s+\(([LRS])\)\s+[A-Z0-9]+$/);
    if (m) {
      players.push(m[1].trim());
    }
  }
