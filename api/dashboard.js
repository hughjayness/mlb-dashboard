...
function getContextScore(candidate, context) {
  const lineupBonus = context.lineupContext.officialLineupAvailable ? 0.8 : -0.8;
...
function getMissingDataPenalty(componentScores) {
  let penalty = 0;
  if (componentScores?.liveFeedStatus?.bullpen !== "live") penalty += 0.55;
  if (componentScores?.liveFeedStatus?.scheduleTravel !== "live") penalty += 0.35;
  if (componentScores?.liveFeedStatus?.offenseVsHand === "placeholder") penalty += 0.45;
  if (componentScores?.liveFeedStatus?.defense === "placeholder") penalty += 0.20;
  return penalty;
}
...
function getThresholdsForBetType(bestBetType, marketFamily, candidateType) {
  ...
  switch (key) {
    case "Side":
      return { high: 10.8, medium: 6.3, minEdgeHigh: 3.5, minEdgeMedium: 1.4 };
    case "Total":
      return { high: 10.6, medium: 6.1, minEdgeHigh: 3.2, minEdgeMedium: 1.3 };
    case "Run Line":
      return { high: 11.4, medium: 6.6, minEdgeHigh: 4.0, minEdgeMedium: 1.8 };
    case "F5 Side":
      return { high: 11.8, medium: 6.8, minEdgeHigh: 4.3, minEdgeMedium: 1.9 };
    case "F5 Total":
      return { high: 11.5, medium: 6.5, minEdgeHigh: 4.0, minEdgeMedium: 1.7 };
    case "F5 Run Line":
      return { high: 12.0, medium: 7.0, minEdgeHigh: 4.5, minEdgeMedium: 2.0 };
    case "prop":
      return /Home Runs/.test(candidateType || "")
        ? { high: 12.3, medium: 6.9, minEdgeHigh: 4.8, minEdgeMedium: 2.0 }
        : { high: 11.2, medium: 6.4, minEdgeHigh: 3.8, minEdgeMedium: 1.7 };
    default:
      return { high: 11.0, medium: 6.2, minEdgeHigh: 3.5, minEdgeMedium: 1.5 };
  }
}
...
function recommendStakeUnits(finalScore, confidence) {
  if (confidence === "High") return finalScore >= 12 ? 1.25 : 1.0;
