  ...
  try {
    ...
    const games = await Promise.all(
      limitedGames.map(async (game, index) => {
        ...
        const propResult = await fetchTopPropForEvent(game.id, apiKey);
    ...
    return res.status(200).json({
      ...
      notes: [
        ...
        "Top prop card now explains more clearly when no supported BetMGM prop was returned."
      ],
      games: games
    });
  } catch (error) {
  ...
  for (const group of propMarketGroups) {
    ...
    try {
      ...
      if (bestProp) {
        ...
        return {
          topProp: bestProp,
          status: "Live BetMGM prop returned."
        };
      }
    } catch (error) {}
  }
