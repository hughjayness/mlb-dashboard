...
function sigmoidScoreToPct(score) {
  ...
  const pct = 100 / (1 + Math.exp(-0.95 * (x - 1.85)));
...
function getMarketPropTilt(marketKey, side, context) {
  ...
  const highRunBoost = runEnvScore > 0 ? runEnvScore * 0.18 : runEnvScore * 0.09;
  const lowRunBoost = runEnvScore < 0 ? Math.abs(runEnvScore) * 0.15 : 0;

  const base =
    marketKey === "pitcher_strikeouts" ? 0.12 :
    marketKey === "pitcher_outs" ? 0.10 :
    marketKey === "batter_total_bases" ? 0.13 :
    marketKey === "batter_home_runs" ? 0.09 :
    marketKey === "batter_stolen_bases" ? 0.09 :
    marketKey === "batter_hits_runs_rbis" ? 0.12 :
  ...
  if (marketKey.startsWith("batter_")) {
    if (side === "over") return round2(base + highRunBoost - (lowRunBoost * 0.35));
    return round2(base * 0.70 + lowRunBoost - (highRunBoost * 0.25));
  }

  if (marketKey.startsWith("pitcher_")) {
    if (side === "over") return round2(base + lowRunBoost - (highRunBoost * 0.40));
    return round2(base * 0.72 + highRunBoost - (lowRunBoost * 0.25));
  }
  ...
  for (const market of safeArray(bookmaker.markets)) {
    ...
    for (const key of Object.keys(bucket)) {
      ...
      if (row.overPrice != null) {
        ...
        const fairProb = clamp((implied || 0.5) + contextTilt * 0.26, 0.05, 0.95);
...
function getPriceDisciplineBonus(price) {
  ...
  if (p >= -120 && p <= 165) return 1.25;
  if (p >= -140 && p <= 225) return 0.95;
  if (p >= -160 && p <= 300) return 0.60;
  if (p < -160 && p >= -210) return -0.05;
  if (p < -210) return -0.18;
  return 0.25;
}

function getNegativeEdgePenalty(candidate) {
  const edge = getSignedEdgePct(candidate);
  return edge >= 0 ? 0 : Math.abs(edge) * 0.65;
}
...
function getConflictSeverity(a, b, context) {
  ...
  if (sa.favoredTeam && sb.pitcherTeam && sb.pitcherSide === "over" && sa.favoredTeam !== sb.pitcherTeam) sev += 0.95;
  if (sb.favoredTeam && sa.pitcherTeam && sa.pitcherSide === "over" && sb.favoredTeam !== sa.pitcherTeam) sev += 0.95;
  if (sa.runEnv && sb.runEnv && sa.runEnv !== sb.runEnv) sev += 0.42;
  if (sa.favoredTeam && sb.favoredTeam && sa.favoredTeam !== sb.favoredTeam) sev += 0.75;
  if (sa.runEnv === "under" && sb.hitterPropSide === "over") sev += 0.30;
  if (sb.runEnv === "under" && sa.hitterPropSide === "over") sev += 0.30;
  if (sa.runEnv === "over" && sb.pitcherSide === "over") sev += 0.15;
  if (sb.runEnv === "over" && sa.pitcherSide === "over") sev += 0.15;
...
function getMissingDataPenalty(componentScores) {
  let penalty = 0;
  if (componentScores?.liveFeedStatus?.probablePitchers !== "live") penalty += 0.03;
  if (componentScores?.liveFeedStatus?.lineup !== "official") penalty += 0.01;
  return penalty;
}
...
function getMarketShapeBonus(candidate, context) {
  ...
  if (bucket === "side") return Math.abs(Number(context?.modelOutputs?.sideComposite || 0)) * 1.00;
  if (bucket === "total") return Math.abs(Number(context?.modelOutputs?.totalComposite || 0)) * 1.00 + marketEnvScore;
  if (bucket === "runline") return Math.abs(Number(context?.modelOutputs?.sideComposite || 0)) * 0.75;
  if (bucket === "f5_side") return Math.abs(Number(context?.modelOutputs?.sideComposite || 0)) * 1.05;
  if (bucket === "f5_total") return Math.abs(Number(context?.modelOutputs?.totalComposite || 0)) * 1.00 + marketEnvScore;
  if (bucket === "f5_runline") return Math.abs(Number(context?.modelOutputs?.sideComposite || 0)) * 0.80;
  if (String(bucket).startsWith("hitter_")) return Math.abs(Number(candidate?.contextTilt || 0)) * 1.00 + marketEnvScore * 0.55;
  if (String(bucket).startsWith("pitcher_")) return Math.abs(Number(candidate?.contextTilt || 0)) * 0.92 + marketEnvScore * 0.45;

  return Math.abs(Number(candidate?.contextTilt || 0)) * 0.75;
}

function getContextScore(candidate, context) {
  const lineupBonus = context?.lineupContext?.officialLineupAvailable ? 0.32 : 0.14;
  const starterKnownBonus =
    context?.probablePitchers?.away !== "TBD" && context?.probablePitchers?.home !== "TBD" ? 0.25 : 0.06;
  ...
  return round2(lineupBonus + starterKnownBonus + marketShapeBonus - missingDataPenalty);
}
...
function getDataCompleteness(componentScores, lineupContext, candidate) {
  ...
  score += lineupContext?.officialLineupAvailable ? 12 : 6;
  score += componentScores?.liveFeedStatus?.probablePitchers === "live" ? 18 : 8;
  ...
  if (String(bucket).startsWith("hitter_")) score += 6;
  if (String(bucket).startsWith("pitcher_")) score += 5;
...
function getThresholdsForCandidate(candidate) {
  ...
  switch (bucket) {
    case "side":
      return { high: 1.40, medium: 0.45, minEdgeHigh: 0.00, minEdgeMedium: -99, minCompletenessHigh: 15 };
    case "total":
      return { high: 1.35, medium: 0.40, minEdgeHigh: 0.00, minEdgeMedium: -99, minCompletenessHigh: 15 };
    case "runline":
      return { high: 1.55, medium: 0.55, minEdgeHigh: 0.00, minEdgeMedium: -99, minCompletenessHigh: 15 };
    case "f5_side":
      return { high: 1.55, medium: 0.55, minEdgeHigh: 0.00, minEdgeMedium: -99, minCompletenessHigh: 15 };
    case "f5_total":
      return { high: 1.60, medium: 0.60, minEdgeHigh: 0.00, minEdgeMedium: -99, minCompletenessHigh: 15 };
    case "f5_runline":
      return { high: 1.70, medium: 0.65, minEdgeHigh: 0.00, minEdgeMedium: -99, minCompletenessHigh: 15 };
    case "pitcher_k_prop":
    case "pitcher_outs_prop":
      return { high: 1.45, medium: 0.50, minEdgeHigh: 0.00, minEdgeMedium: -99, minCompletenessHigh: 12 };
    ...
    case "hitter_walks_prop":
      return { high: 1.20, medium: 0.35, minEdgeHigh: 0.00, minEdgeMedium: -99, minCompletenessHigh: 10 };
    default:
      return { high: 1.30, medium: 0.40, minEdgeHigh: 0.00, minEdgeMedium: -99, minCompletenessHigh: 10 };
  }
}
...
function calibrateCategoryConfidence(rankedCandidates, context) {
  ...
  const categories = [
    { name: "full", test: c => c.marketFamily === "full", floor: 0.85, maxConflict: 0.85 },
    { name: "f5", test: c => c.marketFamily === "f5", floor: 0.85, maxConflict: 0.85 },
    { name: "hitter_props", test: c => c.marketFamily === "prop" && c.propSubType === "hitter", floor: 0.65, maxConflict: 0.75 },
    { name: "pitcher_props", test: c => c.marketFamily === "prop" && c.propSubType === "pitcher", floor: 0.70, maxConflict: 0.75 }
  ];
  ...
  for (const cat of categories) {
    ...
    top.confidence = "High";
    top.confidenceScore = Math.max(Number(top.confidenceScore || 0), 60);
  ...
  for (const candidate of arr.sort((a, b) => Number(b.finalScore || 0) - Number(a.finalScore || 0))) {
    ...
    const conflict = keptHigh.find(other => getConflictSeverity(candidate, other, context) >= 0.95);
    if (conflict) {
      ...
      candidate.conflictPenalty = round2(Number(candidate.conflictPenalty || 0) + 0.95);
...
function calibrateResponseGames(games, viewMode) {
  const playable = safeArray(games)
    .filter(g => g && g.bestBet && g.bestBet !== "Pass" && hasValue(g.bestBetOdds))
    .sort((a, b) => Number(b.confidenceScore || 0) - Number(a.confidenceScore || 0));

  if (!playable.length) return games;

  const hasHigh = playable.some(g => g.confidence === "High");
  const hasMedium = playable.some(g => g.confidence === "Medium");

  if (!hasHigh) {
    playable[0].confidence = "High";
    playable[0].confidenceScore = Math.max(Number(playable[0].confidenceScore || 0), 61);
    playable[0].recommendedTiming = "Bet sooner";
    playable[0].recommendedStakeUnits = 1.0;
    playable[0].reasons = safeArray(playable[0].reasons).concat([`View-level calibration promoted the strongest ${viewMode} candidate.`]);
    playable[0].debug = playable[0].debug || {};
    playable[0].debug.viewCalibratedHigh = true;
  }

  if (!hasMedium && playable.length > 1) {
    playable[1].confidence = playable[1].confidence === "High" ? "High" : "Medium";
    playable[1].confidenceScore = Math.max(Number(playable[1].confidenceScore || 0), 52);
    playable[1].recommendedTiming = playable[1].confidence === "High" ? "Bet sooner" : "Monitor market";
    playable[1].recommendedStakeUnits = playable[1].confidence === "High" ? 1.0 : 0.5;
    playable[1].reasons = safeArray(playable[1].reasons).concat([`View-level calibration promoted a backup ${viewMode} candidate to Medium.`]);
    playable[1].debug = playable[1].debug || {};
    playable[1].debug.viewCalibratedMedium = true;
  }
