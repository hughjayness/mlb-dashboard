// api/dashboard.js
// Full replacement V12.3: Emergency recalibration after current-window drawdown. Pitcher-prop probation, Under-direction gates, plus-money caps, and controlled hitter-prop restoration.
...
const DASHBOARD_MODEL_REVISION = {
  id: "v12-3-emergency-recalibration-2026-06-08",
  date: "2026-06-08",
  label: "V12.3 Emergency Recalibration: pitcher-prop probation, Under-direction discipline, plus-money caps, and controlled hitter-prop restoration"
};
...
function v123HitterQualityGate(g) {
  ...
  const topOrder = Number.isFinite(spot) && spot <= 6;
  const playerSupport = quality >= 0.04 || contact >= 0.08 || power >= 0.08;
  const matchupSupport = oppPitcher <= 0.05 || Number(g?.contextSupportCount || 0) >= 4;
  const cleanRisk = !tags.includes("lineup_not_confirmed") && !tags.includes("bad_lineup_spot_for_over") && !tags.includes("invalid_pick") && !tags.includes("odds_missing_or_invalid");
  if (!cleanRisk || !topOrder) return false;
  if (over && !(playerSupport && matchupSupport)) return false;
  return score >= 3.00 && edge >= 0.90;
}
...
function scoreCandidate(c, ctx) {
  if (isInvalidCandidate(c)) {
    return addConfidenceFields({
      ...
      hybridNotes: ["V12.3 invalid-pick blocker removed this candidate from trust consideration."]
    });
  }
  ...
  // V12.3: split betting value from baseball/professional context, then apply risk gates before confidence.
...
function buildMissedHighReasons(c, thresholds) {
  ...
  if (safeArray(c.riskTags).length) missed.push(`V12.3 risk tags: ${safeArray(c.riskTags).join('; ')}`);
  return missed;
}
...
function confidenceCalibrateCandidates(ranked, ctx) {
  const prime = safeArray(ranked).map(c => {
    ...
    const reason = assigned === "High"
      ? `V12.3 professional-context calibration: value, baseball context, market-specific rules, no-bad-context gates, and same-game script cleared High threshold ${thresholds.high}.`
      : assigned === "Medium"
        ? `V12.3 professional-context calibration: candidate cleared Medium threshold ${thresholds.medium} but did not clear every High gate.`
        : `V12.3 professional-context calibration: candidate was capped or failed professional baseball, market-specific, invalid-pick, context, or historical gates.`;
  ...
  return hybrid.map(c => {
    ...
    const reason = assigned === "High"
      ? `V12.3 ${mode}: professional baseball context, playable price, no-bad-context review, and historical trust/fade review cleared High.`
      : assigned === "Medium"
        ? `V12.3 ${mode}: usable candidate, but one or more professional-context, risk, market-specific, or historical gates kept it below High.`
        : `V12.3 ${mode}: candidate remained Low after professional-context, market-value, risk, and trust/fade/pass review.`;
    return setConfidence({
      ...c,
      highThreshold: thresholds.high,
      mediumThreshold: thresholds.medium,
...
function buildGameResponse(event, ctx, ranked, filtered, viewMode) {
  ...
  const game = {
    ...
    marketAvailability: ctx.marketAvailability,
    debug: {
      confidenceGuarantee: "V12.3 does not blanket-promote every playable row. High requires emergency-recalibrated score/edge/completeness/script gates.",
      selectedBest: best ? {
...
function promoteHitterPropsOnePerGame(g, viewMode) {
  // V12.3: restore controlled hitter-prop coverage without blindly forcing every game to High.
  ...
  const base = {
    ...g,
    trendTags: Array.from(new Set(safeArray(g.trendTags).concat(playable ? ["v12_3_hitter_best_candidate_displayed"] : ["v12_3_no_playable_hitter_prop"]))),
    confidenceForceDebug: {
      forceRan: !!eligibleRestore,
      calibrationMode: "v12_3_controlled_hitter_restoration",
      viewMode,
      reason: eligibleRestore
        ? "V12.3 restored this hitter prop to High only after lineup, edge, player-quality, and risk gates cleared."
        : playable
          ? "One hitter-prop candidate is displayed for the game, but V12.3 only promotes it when controlled quality gates clear."
  ...
  if (eligibleRestore) {
    return setConfidence({
      ...
      decisionMode: "V12.3 Controlled Hitter Restore",
      trustFadePassDecision: "trust",
      reasons: safeArray(base.reasons).concat(["V12.3 controlled hitter-prop restoration: confirmed/top-order hitter context and playable edge restored this candidate to High."])
    }, "High", "V12.3 controlled hitter-prop restoration cleared High gates.");
  }
...
function selectOnePerGameView(games, viewMode) {
  return safeArray(games).map(g => {
    ...
    return addConfidenceFields({
      ...
      confidenceForceDebug: {
        forceRan: false,
        calibrationMode: "v12_3_true_confidence_all_views",
        viewMode,
        reason: "V12.3 uses true confidence after professional-context, market-specific, and no-bad-context gates. No view is forcibly promoted to High."
      }
    });
  });
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      calibrationMode: "v12_3_professional_context_recalibration",
      dashboardRevision: DASHBOARD_MODEL_REVISION,
      oddsDiagnostics
    });
  } catch (err) {
