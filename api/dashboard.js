...
const DASHBOARD_MODEL_REVISION = {
  id: "phase1-data-trends-v11-2026-05-03",
  date: "2026-05-03",
  label: "Phase 1 Data Trends calibration: directional gates, prop-over filters, no blanket High forcing"
};
...
function confidenceCalibrateCandidates(ranked, ctx) {
  const arr = safeArray(ranked).map(c => {
    ...
    const reason = assigned === "High"
      ? `V11 Phase 1 calibration: candidate cleared High threshold ${thresholds.high}, edge minimum ${thresholds.minEdgeHigh}, and same-game script review.`
      : assigned === "Medium"
        ? `V11 Phase 1 calibration: candidate cleared Medium threshold ${thresholds.medium} but did not clear all High gates.`
        : `V11 Phase 1 calibration: candidate did not clear High/Medium gates or was capped by Phase 1 directional/context review.`;
