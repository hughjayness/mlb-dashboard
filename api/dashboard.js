...
function sigmoidScoreToPct(score) {
  ...
  const pct = 100 / (1 + Math.exp(-0.9 * (x - 2.35)));
...
function mergeEventOddsObjects(primary, secondary) {
  ...
  for (const b of safeArray(secondary.bookmakers)) {
    ...
    if (!existing) {
      ...
      for (const m of safeArray(existing.markets)) {
        marketMap.set(String(m.key || Math.random()), m);
      }
      for (const m of safeArray(b.markets)) {
        marketMap.set(String(m.key || Math.random()), m);
      }
...
function getMarketRunEnvironment(totalLine) {
  ...
  if (!Number.isFinite(line)) {
    return { label: "neutral", score: 0, totalLine: null };
  }
  if (line <= 7.5) return { label: "low_run", score: -0.28, totalLine: line };
  if (line >= 9.0) return { label: "high_run", score: 0.30, totalLine: line };
  if (line >= 8.5) return { label: "slightly_high_run", score: 0.15, totalLine: line };
  if (line <= 8.0) return { label: "slightly_low_run", score: -0.12, totalLine: line };
...
function estimateTeamImpliedRuns(context) {
  ...
  if (!Number.isFinite(total)) {
    return { away: null, home: null, diffHomeMinusAway: 0 };
  }
...
function buildModelOutputs(args) {
  ...
  const fairTotal = round2(baseTotal + (totalComposite * 1.15));

  const overProb = clamp(0.5 + ((fairTotal - baseTotal) * 0.12), 0.32, 0.68);
  const underProb = clamp(1 - overProb, 0.32, 0.68);
...
function getMarketPropTilt(marketKey, side, context) {
  ...
  const highRunBoost = runEnvScore > 0 ? runEnvScore * 0.16 : runEnvScore * 0.10;
  const lowRunBoost = runEnvScore < 0 ? Math.abs(runEnvScore) * 0.13 : 0;

  const base =
    marketKey === "pitcher_strikeouts" ? 0.11 :
    marketKey === "pitcher_outs" ? 0.09 :
    marketKey === "batter_total_bases" ? 0.12 :
    marketKey === "batter_home_runs" ? 0.08 :
    marketKey === "batter_stolen_bases" ? 0.08 :
    marketKey === "batter_hits_runs_rbis" ? 0.11 :
    marketKey === "batter_rbis" ? 0.10 :
    marketKey === "batter_runs_scored" ? 0.10 :
    marketKey === "batter_doubles" ? 0.08 :
    marketKey === "batter_triples" ? 0.05 :
    marketKey === "batter_walks" ? 0.07 :
    marketKey === "batter_singles" ? 0.08 :
    0.08;

  if (marketKey.startsWith("batter_")) {
    if (side === "over") return round2(base + highRunBoost - (lowRunBoost * 0.55));
    return round2(base * 0.72 + lowRunBoost - (highRunBoost * 0.35));
  }

  if (marketKey.startsWith("pitcher_")) {
    if (side === "over") return round2(base + lowRunBoost - (highRunBoost * 0.55));
    return round2(base * 0.70 + highRunBoost - (lowRunBoost * 0.35));
  }
  ...
  for (const market of safeArray(bookmaker.markets)) {
    ...
    for (const key of Object.keys(bucket)) {
      ...
      if (row.overPrice != null) {
        ...
        const fairProb = clamp((implied || 0.5) + contextTilt * 0.24, 0.05, 0.95);
        ...
        candidates.push({
          ...
          reasons: [`${marketLabel} over evaluated with market run environment and prop type.`]
        });
      }
      ...
      if (row.underPrice != null) {
        ...
        const fairProb = clamp((implied || 0.5) + contextTilt * 0.18, 0.05, 0.95);
        ...
        candidates.push({
          ...
          contextTilt: round2(contextTilt),
          modelProb: round2(fairProb),
          reasons: [`${marketLabel} under evaluated with market run environment and prop type.`]
        });
      }
    }
  }
...
function getPriceDisciplineBonus(price) {
  ...
  if (p >= -115 && p <= 165) return 1.25;
  if (p >= -135 && p <= 220) return 0.95;
  if (p >= -155 && p <= 275) return 0.55;
  if (p < -155 && p >= -200) return -0.05;
  if (p < -200) return -0.22;
  return 0.22;
}

function getNegativeEdgePenalty(candidate) {
  const edge = getSignedEdgePct(candidate);
  return edge >= 0 ? 0 : Math.abs(edge) * 0.85;
}
...
function getConflictSeverity(a, b, context) {
  ...
  if (sa.favoredTeam && sb.pitcherTeam && sb.pitcherSide === "over" && sa.favoredTeam !== sb.pitcherTeam) sev += 1.05;
  if (sb.favoredTeam && sa.pitcherTeam && sa.pitcherSide === "over" && sb.favoredTeam !== sa.pitcherTeam) sev += 1.05;
  if (sa.runEnv && sb.runEnv && sa.runEnv !== sb.runEnv) sev += 0.52;
  if (sa.favoredTeam && sb.favoredTeam && sa.favoredTeam !== sb.favoredTeam) sev += 0.82;
  if (sa.runEnv === "under" && sb.hitterPropSide === "over") sev += 0.42;
  if (sb.runEnv === "under" && sa.hitterPropSide === "over") sev += 0.42;
  if (sa.runEnv === "over" && sb.pitcherSide === "over") sev += 0.22;
  if (sb.runEnv === "over" && sa.pitcherSide === "over") sev += 0.22;
...
function getProjectedLineupPenalty(candidate, context) {
  ...
  if (["f5_side", "f5_total", "f5_runline", "pitcher_k_prop", "pitcher_outs_prop"].includes(bucket)) return 0.10;
  if (String(bucket).startsWith("hitter_")) return 0.05;
  return 0.02;
}

function getMissingDataPenalty(componentScores) {
  let penalty = 0;
  if (componentScores?.liveFeedStatus?.probablePitchers !== "live") penalty += 0.06;
  if (componentScores?.liveFeedStatus?.lineup !== "official") penalty += 0.02;
  return penalty;
}
...
function getMarketShapeBonus(candidate, context) {
  ...
  if (bucket === "total") return Math.abs(Number(context?.modelOutputs?.totalComposite || 0)) * 0.95 + marketEnvScore;
  ...
  if (bucket === "f5_total") return Math.abs(Number(context?.modelOutputs?.totalComposite || 0)) * 0.95 + marketEnvScore;
  if (bucket === "f5_runline") return Math.abs(Number(context?.modelOutputs?.sideComposite || 0)) * 0.78;
  if (String(bucket).startsWith("hitter_")) return Math.abs(Number(candidate?.contextTilt || 0)) * 0.80 + marketEnvScore * 0.45;
  if (String(bucket).startsWith("pitcher_")) return Math.abs(Number(candidate?.contextTilt || 0)) * 0.78 + marketEnvScore * 0.35;

  return Math.abs(Number(candidate?.contextTilt || 0)) * 0.65;
}

function getContextScore(candidate, context) {
  const lineupBonus = context?.lineupContext?.officialLineupAvailable ? 0.32 : 0.10;
  const starterKnownBonus =
    context?.probablePitchers?.away !== "TBD" && context?.probablePitchers?.home !== "TBD" ? 0.25 : 0.02;
...
function getDataCompleteness(componentScores, lineupContext, candidate) {
  ...
  if (lineupContext?.officialLineupAvailable) score += 12;
  else score += 5;

  if (componentScores?.liveFeedStatus?.probablePitchers === "live") score += 18;
  else score += 7;
  ...
  if (String(bucket).startsWith("hitter_")) score += 5;
  if (String(bucket).startsWith("pitcher_")) score += 4;
...
function getThresholdsForCandidate(candidate) {
  ...
  switch (bucket) {
    case "side":
      return { high: 2.00, medium: 0.95, minEdgeHigh: 0.06, minEdgeMedium: 0.00, minCompletenessHigh: 25 };
    case "total":
      return { high: 1.95, medium: 0.90, minEdgeHigh: 0.06, minEdgeMedium: 0.00, minCompletenessHigh: 25 };
    case "runline":
      return { high: 2.25, medium: 1.05, minEdgeHigh: 0.08, minEdgeMedium: 0.00, minCompletenessHigh: 25 };
    case "f5_side":
      return { high: 2.25, medium: 1.05, minEdgeHigh: 0.08, minEdgeMedium: 0.00, minCompletenessHigh: 25 };
    case "f5_total":
      return { high: 2.30, medium: 1.10, minEdgeHigh: 0.08, minEdgeMedium: 0.00, minCompletenessHigh: 25 };
    case "f5_runline":
      return { high: 2.40, medium: 1.15, minEdgeHigh: 0.10, minEdgeMedium: 0.00, minCompletenessHigh: 25 };
    case "pitcher_k_prop":
    case "pitcher_outs_prop":
      return { high: 2.10, medium: 1.00, minEdgeHigh: 0.05, minEdgeMedium: 0.00, minCompletenessHigh: 22 };
    ...
    case "hitter_walks_prop":
      return { high: 1.75, medium: 0.80, minEdgeHigh: 0.00, minEdgeMedium: 0.00, minCompletenessHigh: 20 };
    default:
      return { high: 1.90, medium: 0.85, minEdgeHigh: 0.00, minEdgeMedium: 0.00, minCompletenessHigh: 20 };
  }
}
...
function assignConfidenceForCandidate(candidateLike) {
  ...
  if (candidateLike.finalScore >= thresholds.medium && edge >= thresholds.minEdgeMedium) {
    return "Medium";
  }
...
function calibrateCategoryConfidence(rankedCandidates, context) {
  ...
  const categories = [
    { name: "full", test: c => c.marketFamily === "full", floor: 1.55, maxConflict: 0.70 },
    { name: "f5", test: c => c.marketFamily === "f5", floor: 1.65, maxConflict: 0.65 },
    { name: "hitter_props", test: c => c.marketFamily === "prop" && c.propSubType === "hitter", floor: 1.35, maxConflict: 0.55 },
    { name: "pitcher_props", test: c => c.marketFamily === "prop" && c.propSubType === "pitcher", floor: 1.45, maxConflict: 0.60 }
  ];
  ...
  for (const cat of categories) {
    ...
    top.confidence = "High";
    top.confidenceScore = Math.max(Number(top.confidenceScore || 0), 58);
    top.recommendedTiming = "Bet sooner";
    top.recommendedStakeUnits = 1.0;
    top.calibratedHigh = true;
    top.reasons = safeArray(top.reasons).concat(["Category calibration promoted the top non-conflicted pick."]);
  }
  ...
  for (const candidate of arr.sort((a, b) => Number(b.finalScore || 0) - Number(a.finalScore || 0))) {
    ...
    const conflict = keptHigh.find(other => getConflictSeverity(candidate, other, context) >= 0.85);
    if (conflict) {
      candidate.confidence = "Medium";
      candidate.confidenceScore = Math.min(Number(candidate.confidenceScore || 50), 57);
      candidate.recommendedTiming = "Monitor market";
      candidate.recommendedStakeUnits = 0.5;
      candidate.conflictPenalty = round2(Number(candidate.conflictPenalty || 0) + 0.85);
...
function addMissedHighReasons(candidates) {
  return safeArray(candidates).map(c => {
    ...
    if (c.confidence !== "High") {
      ...
      if (getPositiveEdgePct(c) < thresholds.minEdgeHigh) missed.push(`edge below high minimum (${getPositiveEdgePct(c)} < ${thresholds.minEdgeHigh})`);
  ...
  return {
    ...
    debug: {
      ...
      dataAvailability: {
        ...
        structuralSignals: {
          probablePitchers: context.componentScores?.liveFeedStatus?.probablePitchers,
          parkFactor: context.componentScores?.liveFeedStatus?.parkFactor,
          teamStrength: context.componentScores?.liveFeedStatus?.teamStrength,
          marketContext: context.componentScores?.liveFeedStatus?.marketContext,
          runEnvironment: context.componentScores?.market?.runEnvironment
        }
      }
    }
  };
...
async function buildGameContext(event, apiKey, scheduleGames, viewMode) {
  ...
  if (["props", "hitter_props", "pitcher_props"].includes(viewMode)) {
    ...
    if (hitterNeeded) {
      ...
      hitterPropOdds = mergeEventOddsObjects(mergeEventOddsObjects(hitterA, hitterB), hitterC);
    }

    if (pitcherNeeded) {
      pitcherPropOdds = await fetchEventMarkets(event.id, apiKey, PITCHER_PROP_MARKETS);
    }
