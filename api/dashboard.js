  ...
  try {
    ...
    const games = await Promise.all(
      limitedGames.map(async (game, index) => {
        ...
        const firstFiveData = await fetchFirstFiveMarketsForEvent({
          eventId: game.id,
          apiKey,
          homeTeam,
          awayTeam
...
function buildRecommendation({
  ...
  if (firstFiveData && firstFiveData.moneyline && typeof firstFiveModelOutputs.homeEdgePct === "number") {
    gameCandidates.push({
      bestBet: `${homeTeam} F5 ML`,
      bestBetType: "F5 Side",
      edge: firstFiveModelOutputs.homeEdgePct,
      reasons: buildFirstFiveSideReasons(homeTeam, "home", firstFiveModelOutputs, componentScores, firstFiveData.moneyline),
      scoreForConfidence: Math.abs(firstFiveModelOutputs.homeEdgePct)
    });
  }

  if (firstFiveData && firstFiveData.moneyline && typeof firstFiveModelOutputs.awayEdgePct === "number") {
    gameCandidates.push({
      bestBet: `${awayTeam} F5 ML`,
      bestBetType: "F5 Side",
      edge: firstFiveModelOutputs.awayEdgePct,
      reasons: buildFirstFiveSideReasons(awayTeam, "away", firstFiveModelOutputs, componentScores, firstFiveData.moneyline),
      scoreForConfidence: Math.abs(firstFiveModelOutputs.awayEdgePct)
    });
  }

  if (firstFiveData && firstFiveData.total && firstFiveData.total.point !== null && typeof firstFiveModelOutputs.overEdgePct === "number") {
    gameCandidates.push({
      bestBet: `F5 Over ${firstFiveData.total.point}`,
      bestBetType: "F5 Total",
      edge: firstFiveModelOutputs.overEdgePct,
      reasons: buildFirstFiveTotalReasons("Over", firstFiveData.total.point, firstFiveModelOutputs, componentScores),
      scoreForConfidence: Math.abs(firstFiveModelOutputs.overEdgePct)
    });
  }

  if (firstFiveData && firstFiveData.total && firstFiveData.total.point !== null && typeof firstFiveModelOutputs.underEdgePct === "number") {
    gameCandidates.push({
      bestBet: `F5 Under ${firstFiveData.total.point}`,
      bestBetType: "F5 Total",
      edge: firstFiveModelOutputs.underEdgePct,
      reasons: buildFirstFiveTotalReasons("Under", firstFiveData.total.point, firstFiveModelOutputs, componentScores),
      scoreForConfidence: Math.abs(firstFiveModelOutputs.underEdgePct)
    });
  }

  if (firstFiveData && firstFiveData.spread && firstFiveData.spread.homePoint !== null && firstFiveData.spread.awayPoint !== null) {
    if (typeof firstFiveModelOutputs.homeEdgePct === "number" && firstFiveModelOutputs.homeEdgePct > 0 && firstFiveData.spread.homePoint < 0) {
      gameCandidates.push({
        bestBet: `${homeTeam} F5 ${formatSpread(firstFiveData.spread.homePoint)}`,
        bestBetType: "F5 Run Line",
        edge: round1(firstFiveModelOutputs.homeEdgePct - 0.4),
        reasons: buildFirstFiveRunLineReasons(homeTeam, firstFiveData.spread.homePoint, "home", firstFiveModelOutputs, componentScores),
        scoreForConfidence: Math.abs(round1(firstFiveModelOutputs.homeEdgePct - 0.4))
      });
    }
    if (typeof firstFiveModelOutputs.awayEdgePct === "number" && firstFiveModelOutputs.awayEdgePct > 0 && firstFiveData.spread.awayPoint < 0) {
      gameCandidates.push({
        bestBet: `${awayTeam} F5 ${formatSpread(firstFiveData.spread.awayPoint)}`,
        bestBetType: "F5 Run Line",
        edge: round1(firstFiveModelOutputs.awayEdgePct - 0.4),
        reasons: buildFirstFiveRunLineReasons(awayTeam, firstFiveData.spread.awayPoint, "away", firstFiveModelOutputs, componentScores),
        scoreForConfidence: Math.abs(round1(firstFiveModelOutputs.awayEdgePct - 0.4))
      });
    }
  }
