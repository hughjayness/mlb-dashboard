  ...
  try {
    ...
    const games = await Promise.all(
      limitedGames.map(async (game, index) => {
        ...
        const scheduleContext = await fetchScheduleContextForDate(dateEt);
        const gameContext = findGameContext(scheduleContext, awayTeam, homeTeam);
    ...
    return res.status(200).json({
      ...
      notes: [
        ...
        "Probable pitchers and team season stats are now used to build meaningful driver inputs.",
        "Props are optional challengers and may be skipped for low-value spots to improve performance."
      ],
      games
    });
  } catch (error) {
...
async function fetchTeamSeasonStatsMap() {
  ...
  const url = "https://statsapi.mlb.com/api/v1/teams?sportId=1&hydrate=stats(group=[hitting,pitching],type=[season])";
  ...
  try {
    ...
    teams.forEach(team => {
      const name = normalizeTeamName(team && team.name);
      if (!name) return;

      const hitting = extractTeamGroupStats(team, "hitting");
      const pitching = extractTeamGroupStats(team, "pitching");
...
function extractTeamGroupStats(team, groupName) {
  const statsBlocks = Array.isArray(team && team.stats) ? team.stats : [];
  for (const block of statsBlocks) {
    const groupDisplay = block && block.group && (block.group.displayName || block.group.name);
    const splits = Array.isArray(block && block.splits) ? block.splits : [];
    if (!groupDisplay || groupDisplay.toLowerCase() !== groupName.toLowerCase()) continue;
...
async function fetchScheduleContextForDate(dateEt) {
  ...
  const url =
    "https://statsapi.mlb.com/api/v1/schedule?sportId=1&date=" + encodeURIComponent(dateEt) +
    "&hydrate=probablePitcher(stats(type=[season],group=[pitching]))";
...
function findGameContext(scheduleJson, awayTeam, homeTeam) {
  const out = {
  ...
  for (const date of dates) {
    ...
    for (const game of games) {
      ...
      out.awayProbablePitcher = extractProbablePitcher(game && game.teams && game.teams.away && game.teams.away.probablePitcher);
      out.homeProbablePitcher = extractProbablePitcher(game && game.teams && game.teams.home && game.teams.home.probablePitcher);
      return out;
    }
  }
...
function extractProbablePitcher(pitcher) {
  if (!pitcher) return null;

  const statsBlocks = Array.isArray(pitcher.stats) ? pitcher.stats : [];
  let stat = {};
  for (const block of statsBlocks) {
    const splits = Array.isArray(block && block.splits) ? block.splits : [];
    if (splits[0] && splits[0].stat) {
      stat = splits[0].stat;
      break;
    }
  }
  ...
  return {
    ...
    inningsPitched: inningsToDecimal(stat.inningsPitched),
...
function buildModelOutputs({ componentScores, moneylineData, totalData, lineupContext }) {
  const sideComposite =
...
function parseSpreads(market, homeTeam, awayTeam) {
  ...
  return {
    ...
    homePoint: awayNegateFix(home.point),
    awayPoint: awayNegateFix(away.point),
    homePrice: home.price,
    awayPrice: away.price,
    homeProb: americanToProb(home.price),
    awayProb: americanToProb(away.price)
...
function awayNegateFix(point) {
  return point === null || point === undefined ? null : Number(point);
}
