// api/dashboard.js
// Full replacement V3: hard guards against fake zero markets + guaranteed slate confidence for valid playable candidates.
...
function forceViewConfidence(games, viewMode) {
  ...
  if (!playable.length) {
    return arr.map(g => addConfidenceFields({
      ...g,
      confidence: "Low",
      confidenceForceDebug: { forceRan: true, viewMode, playableRows: 0, reason: "No non-Pass playable rows existed for this view." }
    }));
  }

  for (const g of playable) {
    Object.assign(g, setConfidence(g, "Low", "View guarantee reset before final ranking."));
  }

  Object.assign(playable[0], setConfidence(playable[0], "High", `View guarantee FINAL: strongest playable  row forced to High Confidence.`));

  if (playable.length > 1) {
    Object.assign(playable[1], setConfidence(playable[1], "Medium", `View guarantee FINAL: second playable  row forced to Medium Confidence.`));
  }
  ...
  return arr.map(g => addConfidenceFields({
    ...
    confidenceForceDebug: {
      ...
      forcedHighBet: playable[0]?.bestBet || null,
      forcedMediumBet: playable[1]?.bestBet || null
    }
  }));
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      calibrationMode: "v3_hard_view_force_no_odds_gate"
    });
  } catch (err) {
