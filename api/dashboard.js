...
async function fetchTopPropForEvent(eventId, apiKey) {
  // Start small. These are common prop-style markets to try first.
  // Coverage varies by book and event.
  const propMarkets = [
    "batter_home_runs",
    "batter_hits",
    "batter_total_bases",
...
function scorePropOutcome(marketKey, outcome) {
  // Favor Over props and shorter prices for now.
  // This is still simple phase-1 logic.
  let score = 0;
...
function buildPropReasons(marketKey, outcome) {
  ...
  if (outcome.name === "Over") {
    reasons.push("Model currently prefers the over side of this prop");
  } else if (outcome.name === "Under") {
    reasons.push("Model currently prefers the under side of this prop");
  }

  if (outcome.price < 0) {
    reasons.push(`Book is pricing this as a favored outcome at ${formatAmerican(outcome.price)}`);
  } else {
    reasons.push(`Book is pricing this as a plus-money outcome at ${formatAmerican(outcome.price)}`);
  }

  switch (marketKey) {
    case "pitcher_strikeouts":
      reasons.push("Pitcher strikeout props are a good early prop category to track");
      break;
    case "batter_total_bases":
      reasons.push("Total bases props are useful for power-hitter evaluation");
      break;
    case "batter_hits":
      reasons.push("Hits props are typically more stable than home run props");
      break;
    case "batter_home_runs":
      reasons.push("Home run props have higher variance than most prop types");
      break;
  }
...
function evaluateGame({ homeTeam, awayTeam, moneylineData, spreadData, totalData, topProp }) {
  ...
  if (moneylineData.homeProb !== null && moneylineData.awayProb !== null) {
    ...
    if (moneylineData.homeProb > moneylineData.awayProb && homeEdge > bestScore) {
      ...
      reasons.push("This evaluation is based on live market pricing");
    }
  ...
  if (bestBetType === "Pass" && spreadData.homePoint !== null && spreadData.awayPoint !== null) {
    if (spreadData.homePoint < 0) {
      ...
      reasons.push("Bookmaker spread market indicates a stronger home side");
      ...
      reasons.push("Bookmaker spread market indicates a stronger away side");
      reasons.push("This is a weaker signal than a strong moneyline split");
    }
  }
  ...
  if (topProp && topProp.modelProb !== null) {
    ...
    if (propScore > bestScore) {
      ...
      reasons.push("Prop evaluation is currently market-based, not stat-model based");
    }
  }
