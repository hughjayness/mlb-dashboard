...
function selectOnePerGameView(games, viewMode) {
  return safeArray(games).map(g => addConfidenceFields({
    ...g,
    confidenceForceDebug: {
      forceRan: false,
      calibrationMode: "v11_phase1_true_confidence_no_blanket_high",
      viewMode,
      reason: "One displayed candidate per game/view is returned, but confidence is not forcibly promoted. Archive should save only true High rows."
    }
  }));
}
