...
async function fetchFeaturedOdds(apiKey) {
  const attempts = [
    `${ODDS_API_BASE}/sports/baseball_mlb/odds?regions=us,us2,uk,eu,au&oddsFormat=american&markets=${encodeURIComponent(FEATURED_MARKETS.join(","))}&apiKey=${encodeURIComponent(apiKey)}`,
    `${ODDS_API_BASE}/sports/baseball_mlb/odds?regions=us,us2,uk,eu,au&oddsFormat=american&markets=h2h&apiKey=${encodeURIComponent(apiKey)}`,
    `${ODDS_API_BASE}/sports/baseball_mlb/odds?oddsFormat=american&markets=${encodeURIComponent(FEATURED_MARKETS.join(","))}&apiKey=${encodeURIComponent(apiKey)}`,
    `${ODDS_API_BASE}/sports/baseball_mlb/odds?oddsFormat=american&markets=h2h&apiKey=${encodeURIComponent(apiKey)}`
  ];
...
async function buildGameContext(event, apiKey, scheduleGames) {
  ...
  const f5Odds = event.id ? await fetchEventMarkets(event.id, apiKey, F5_MARKETS) : null;
  const propOdds = event.id ? await fetchEventMarkets(event.id, apiKey, PROP_MARKETS) : null;
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    for (const e of safeArray(featuredOdds)) {
      const key = `${normalizeTeamName(e?.away_team)}|${normalizeTeamName(e?.home_team)}`;
