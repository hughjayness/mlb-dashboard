...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const games = await Promise.all(limitedGames.map(async function(game, index) {
      ...
      const h2hMarket = markets.find(function(m) { return m && m.key === "h2h"; });
      const spreadMarket = markets.find(function(m) { return m && m.key === "spreads"; });
      const totalMarket = markets.find(function(m) { return m && m.key === "totals"; });
...
function buildModelOutputs(args) {
  ...
  const homeWinProb = clamp(0.5 + (sideScore * 0.08), 0.35, 0.65);
  ...
  const fairTotal = round2(baseTotal + (totalScore * 0.65) + ((parkFactor - 1.0) * 1.2));
...
function scoreCandidates(context, candidates) {
  return (Array.isArray(candidates) ? candidates : []).map(function(candidate) {
    ...
    const dataCompleteness = getDataCompleteness(candidate, context);
    const finalScore = round2((valueScore * 0.56) + (contextScore * 0.29) + (dataCompleteness * 0.15));
...
function getValueScore(candidate) {
  ...
  const fairProbBonus =
    candidate.fairProb != null && candidate.impliedProb != null
      ? Math.max(0, (candidate.fairProb - candidate.impliedProb) * 4.5)
      : 0;

  return round2((edge * 0.95) + juiceBonus + fairProbBonus);
}

function getContextScore(candidate, context) {
  const lineupBonus = context.lineupContext && context.lineupContext.officialLineupAvailable ? 1.0 : -0.2;
...
function getDataCompleteness(candidate, context) {
  let score = 1.8;

  if (candidate.marketFamily === "prop") {
    if (context.lineupContext && context.lineupContext.officialLineupAvailable) score += 1.1;
    else score -= 0.45;
  }

  if (candidate.impliedProb != null) score += 0.65;
  if (candidate.fairProb != null) score += 0.55;
  if (context.probablePitchers && context.probablePitchers.away !== "TBD" && context.probablePitchers.home !== "TBD") score += 0.45;

  return round2(clamp(score, 0.4, 4.5));
}

function getMissingDataPenalty(componentScores) {
  let penalty = 0;
  if (componentScores && componentScores.liveFeedStatus && componentScores.liveFeedStatus.bullpen !== "live") penalty += 0.18;
  if (componentScores && componentScores.liveFeedStatus && componentScores.liveFeedStatus.scheduleTravel !== "live") penalty += 0.14;
...
function getPriceDisciplineBonus(price) {
  ...
  if (p >= -115 && p <= 135) return 1.05;
  if (p >= -130 && p <= 160) return 0.7;
  if (p < -160) return -0.2;
  return 0.35;
}
...
function getMarketShapeBonus(candidate, context) {
  const lineupOfficial = !!(context.lineupContext && context.lineupContext.officialLineupAvailable);

  if (candidate.marketFamily === "prop") {
    if (candidate.propSubType === "hitter") return lineupOfficial ? 0.95 : -0.35;
    if (candidate.propSubType === "pitcher") return lineupOfficial ? 0.85 : -0.25;
  }

  if (candidate.marketFamily === "f5") return 0.55;
  if (candidate.marketFamily === "full") return 0.32;
  return 0;
}

function scoreToConfidence(finalScore) {
  if (finalScore >= 5.35) return "High";
  if (finalScore >= 3.35) return "Medium";
  return "Low";
}
...
function getRecommendedStakeUnits(finalScore) {
  if (finalScore >= 6.0) return 1.5;
  if (finalScore >= 5.0) return 1.0;
  if (finalScore >= 3.5) return 0.75;
  return 0.5;
}
...
function buildGamePayload(args) {
  ...
  return {
    ...
    firstFiveMoneylineConfidence: deriveF5ConfidenceMap(scoredCandidates, "f5_moneyline", args.awayTeam, args.homeTeam),
    firstFiveTotalConfidence: deriveF5TotalConfidenceMap(scoredCandidates),
    firstFiveRunLineConfidence: deriveF5RunLineConfidenceMap(scoredCandidates, args.awayTeam, args.homeTeam),
...
function deriveF5ConfidenceMap(candidates, market, awayTeam, homeTeam) {
...
function deriveF5RunLineConfidenceMap(candidates, awayTeam, homeTeam) {
