...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      mode: viewMode,
      notes: [
        "Official lineup composition is now used in hitter-prop and pitcher-prop scoring.",
...
function parseMLBStartingLineups(html) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (i + 2 < lines.length && lines[i + 1] === "@") {
      ...
      const block = lines.slice(i, Math.min(i + 260, lines.length));
...
function buildLineupContext({ homeTeam, awayTeam, mlbLineups, probablePitchers, scheduleGame }) {
  ...
  const officialLineups = hasOfficial
    ? {
        away: { team: awayTeam, status: "official", players: official.awayPlayers },
        home: { team: homeTeam, status: "official", players: official.homePlayers }
      }
    : { away: null, home: null };
...
async function fetchPropCandidatesForEventCached(eventId, apiKey, lineupContext) {
...
function buildPropCandidateFromOutcome(marketKey, outcome, lineupContext) {
  ...
  const playerMeta = findPlayerInOfficialLineups(playerName, lineupContext);
  const officialLineupBonus = lineupContext.officialLineupAvailable ? 0.8 : -0.25;
  const lineupSlotBonus = getLineupSlotBonus(playerMeta?.lineupIndex);
  const platoonBonus = getPlatoonBonus(playerMeta?.bats, getOpposingPitcherHand(playerMeta?.teamSide, lineupContext));
  const teamEnvBonus = getTeamEnvironmentBonus(playerMeta?.teamSide, lineupContext);
  const marketBias = getPropMarketBias(marketKey, outcome);
  const priceDiscipline = getPriceDisciplineBonus(outcome.price);

  let contextTilt = officialLineupBonus + lineupSlotBonus + platoonBonus + teamEnvBonus + marketBias + priceDiscipline;

  if (marketKey === "pitcher_strikeouts" || marketKey === "pitcher_outs") {
    contextTilt += getPitcherPropLineupAdjustment(playerName, marketKey, lineupContext);
  }

  const fairProb = clamp(marketProb + (contextTilt * 0.0125), 0.08, 0.92);
  const edgePct = round1((fairProb - marketProb) * 100);
  const preScore = round2(Math.abs(edgePct) + contextTilt * 1.2);
  ...
  return {
    ...
    reasons: buildPropReasonsV2(marketKey, outcome, lineupContext, edgePct, fairProb, playerMeta),
    candidateMeta: {
      player: playerName,
      market: marketLabel,
      side: outcome.name,
...
function buildComponentScores({ moneylineData, totalData, lineupContext, parkFactor, probablePitchers }) {
  ...
  const lineupStrength = getLineupStrengthTilt(lineupContext);
  const lineupScore = lineupContext.officialLineupAvailable ? round2(0.40 + Math.abs(lineupStrength) * 0.20) : 0.12;
  ...
  return {
    side: {
      ...
      bullpen: 0,
      lineup: lineupScore,
      offenseVsHand: round2(lineupStrength * 0.5),
      defense: 0,
    ...
    total: {
      ...
      offenseQuality: round2(Math.abs(lineupStrength) * 0.25),
      lineups: lineupScore,
    ...
    liveFeedStatus: {
      ...
      offenseVsHand: lineupContext.officialLineupAvailable ? "lineup-informed" : "placeholder",
      defense: "placeholder",
...
function buildModelOutputs({ componentScores, moneylineData, totalData, parkFactor }) {
  const sideComposite =
    (0.28 * componentScores.side.startingPitcher) +
    (0.10 * componentScores.side.bullpen) +
    ...
    (0.04 * componentScores.side.scheduleTravel) +
    (0.12 * componentScores.side.marketContext);

  const totalComposite =
    (0.18 * componentScores.total.starterRunSuppression) +
    (0.12 * componentScores.total.bullpenRunSuppression) +
    (0.18 * componentScores.total.offenseQuality) +
    (0.12 * componentScores.total.lineups) +
  ...
  if (marketTotalLine !== null) {
    fairTotal = round1(marketTotalLine + (totalComposite * 0.38));
  }
...
function generateAllCandidates(context) {
  const out = [];
  const { awayTeam, homeTeam, moneylineData, totalData, spreadData, modelOutputs, propCandidates, viewMode } = context;
  ...
  if (spreadData.awayPrice != null && spreadData.awayPoint != null) {
    out.push({
      ...
      reasons: buildRunLineReasons(awayTeam, spreadData.awayPoint, context)
    });
  }
  ...
  if (spreadData.homePrice != null && spreadData.homePoint != null) {
    out.push({
      ...
      reasons: buildRunLineReasons(homeTeam, spreadData.homePoint, context)
    });
  }
...
function scoreCandidates(context, candidates) {
  return (Array.isArray(candidates) ? candidates : []).map(candidate => {
    ...
    const thresholds = getThresholdsForBetType(candidate.bestBetType, candidate.marketFamily, candidate.candidateType);
    const finalScore = round2((0.56 * valueScore) + (0.44 * contextScore) + (completenessScore / 20));

    const candidateWithMeta = Object.assign({}, candidate, {
    ...
    const confidence = determineConfidenceTier(finalScore, thresholds, candidateWithMeta, context);

    return Object.assign({}, candidateWithMeta, {
      confidence,
...
function getPropMarketBias(marketKey, outcome) {
  let bias = 0;
  if (marketKey === "pitcher_strikeouts") bias += outcome.name === "Under" ? 0.55 : 0.25;
  if (marketKey === "pitcher_outs") bias += outcome.name === "Under" ? 0.45 : 0.20;
  if (marketKey === "batter_hits") bias += outcome.name === "Over" ? 0.40 : 0.20;
  if (marketKey === "batter_total_bases") bias += outcome.name === "Over" ? 0.32 : 0.18;
  if (marketKey === "batter_home_runs") bias += outcome.name === "Over" ? -0.65 : -0.90;

  if (typeof outcome.point === "number") {
    if (marketKey === "pitcher_strikeouts" && outcome.name === "Over" && outcome.point >= 7.5) bias -= 0.40;
    if (marketKey === "pitcher_strikeouts" && outcome.name === "Under" && outcome.point >= 7.5) bias += 0.15;
    if (marketKey === "batter_hits" && outcome.point >= 1.5 && outcome.name === "Over") bias -= 0.15;
    if (marketKey === "batter_total_bases" && outcome.point >= 2.5 && outcome.name === "Over") bias -= 0.22;
  }
...
function getPriceDisciplineBonus(price) {
  ...
  if (p > 165) return 0.5;
  if (p <= -170) return -0.8;
  if (p <= -140) return -0.30;
  return 0.2;
}

function buildPropReasonsV2(marketKey, outcome, lineupContext, edgePct, fairProb, playerMeta) {
  const reasons = [];
  ...
  if (playerMeta?.lineupIndex) reasons.push(`Player is in lineup spot ${playerMeta.lineupIndex}.`);
  if (playerMeta?.bats) reasons.push(`Bats ${playerMeta.bats}.`);
  const oppHand = getOpposingPitcherHand(playerMeta?.teamSide, lineupContext);
  if (oppHand && playerMeta?.bats && /^batter_/.test(marketKey)) reasons.push(`Opposing starter throws ${oppHand}.`);

  switch (marketKey) {
    case "pitcher_strikeouts":
      reasons.push("Pitcher strikeout props are adjusted for the actual opposing lineup composition.");
      break;
    case "pitcher_outs":
      reasons.push("Pitcher outs props are adjusted for actual lineup depth and top-order pressure.");
      break;
    case "batter_hits":
      reasons.push("Hitter hits props now use confirmed lineup slot and opposing starter handedness.");
      break;
    case "batter_total_bases":
      reasons.push("Total bases props now use confirmed lineup slot and opposing starter handedness.");
      break;
    case "batter_home_runs":
      reasons.push("Home run props remain high variance, but lineup and matchup still feed the score.");
      break;
  }
...
function buildSideReasons(teamName, side, modelOutputs, componentScores, moneylineData) {
  ...
  return [
    ...
    componentScores.liveFeedStatus.lineup === "official"
      ? "Official batting orders are being included in the context score."
      : "Official batting orders are not yet confirmed."
  ];
}
...
function buildTotalReasons(direction, line, modelOutputs) {
  ...
  return [
    ...
    direction === "Under"
      ? "Under is explicitly scored, not treated as a fallback."
      : "Over is explicitly scored against the same baseline as the under."
  ];
}

function buildRunLineReasons(teamName, point, context) {
  return [
...
function buildRiskWarnings(componentScores, bestCandidate, candidates, viewMode) {
  const warnings = [];
  ...
  if (viewMode === "f5") warnings.push("F5 mode still requires a separate F5 market feed before it can become a fully distinct board.");
  return warnings;
}
...
function convertCandidateToTopProp(candidate, lineupContext) {
  ...
  return {
    ...
    market: candidate.candidateMeta?.market || candidate.bestBet,
