// api/dashboard.js
// Full replacement V11: Phase 1 Data Trends calibration, directional gates, prop-over filters, and no blanket High forcing.
...
const DASHBOARD_MODEL_REVISION = {
  id: "v12-hybrid-professional-context-2026-05-23",
  date: "2026-05-23",
  label: "V12 Hybrid Trust/Fade + professional baseball-context scoring: hitter quality, opponent offense difficulty, targeted books"
};
...
function buildHitterProfessionalContext(c, ctx) {
  ...
  let propFit = 0;
  if (c.market === "batter_total_bases") propFit = q.power * 0.58 + q.contact * 0.28;
  else if (c.market === "batter_hits") propFit = q.contact * 0.72 + q.plateSkill * 0.20;
  else if (c.market === "batter_home_runs") propFit = q.power * 0.90;
  else if (c.market === "batter_rbis" || c.market === "batter_runs_scored" || c.market === "batter_hits_runs_rbis") propFit = q.total * 0.45 + ownOffense * 0.55;
  else if (c.market === "batter_walks") propFit = q.plateSkill * 0.80 + q.contact * 0.10;
  ...
  if (c.side === "over") {
    ...
    if (info?.lineupSpot != null && info.lineupSpot >= 7) gateFailures.push("hitter is buried in the lineup");
    if (c.market === "batter_total_bases" && q.power < 0.05) gateFailures.push("power/extra-base support is weak for Total Bases Over");
    if (c.market === "batter_hits" && q.contact < 0.02) gateFailures.push("contact support is weak for Hits Over");
    if (overScore < 0.10) gateFailures.push("professional hitter-context score is not strong enough for Over");
  }
...
function scoreCandidate(c, ctx) {
  ...
  // V12 deliberately reduces odds dominance. Price/value still matters, but baseball context must carry the pick.
  const marketValueScore = round2(edge * 0.10 + getPriceDisciplineScore(c) * 0.72);
  ...
  return addConfidenceFields({
    ...
    professionalConfidenceCap: professional.confidenceCap || null,
...
function candidateThresholds(c) {
  ...
  if (isHitter) { high = 3.42; medium = 2.42; minEdgeHigh = 1.65; minEdgeMedium = 0.25; minCompletenessHigh = 72; }
...
function assignBalancedConfidence(c) {
  ...
  if (score >= t.high && edge >= t.minEdgeHigh && completeness >= t.minCompletenessHigh && c.professionalConfidenceCap !== "Medium") return "High";
...
function confidenceCalibrateCandidates(ranked, ctx) {
  const prime = safeArray(ranked).map(c => {
    ...
    const reason = assigned === "High"
      ? `V12 professional-context calibration: baseball-quality, market-value, professional gates, and same-game script cleared High threshold ${thresholds.high}.`
      : assigned === "Medium"
        ? `V12 professional-context calibration: candidate cleared Medium threshold ${thresholds.medium} but did not clear every High gate.`
        : `V12 professional-context calibration: candidate was capped or failed professional baseball/market/historical gates.`;
  ...
  return hybrid.map(c => {
    ...
    const reason = assigned === "High"
      ? `V12 ${mode}: professional baseball context, playable price, and historical trust/fade review cleared High.`
      : assigned === "Medium"
        ? `V12 ${mode}: usable candidate, but one or more professional-context or historical gates kept it below High.`
        : `V12 ${mode}: candidate remained Low after professional-context, market-value, and trust/fade/pass review.`;
    return setConfidence({
      ...c,
      highThreshold: thresholds.high,
      mediumThreshold: thresholds.medium,
...
function buildGameResponse(event, ctx, ranked, filtered, viewMode) {
  ...
  const game = {
    ...
    marketAvailability: ctx.marketAvailability,
    debug: {
      confidenceGuarantee: "V11 does not blanket-promote every playable row. High requires Phase 1 score/edge/completeness/script gates.",
      selectedBest: best ? {
...
function promoteHitterPropsOnePerGame(g, viewMode) {
  ...
  if (!playable) {
    return addConfidenceFields({
      ...
      confidenceForceDebug: {
        forceRan: false,
        calibrationMode: "v12_1_hitter_one_per_game_when_playable",
        viewMode,
        reason: "No hitter-prop candidate was playable for this game, so no one-per-game High selection was forced."
      }
    });
  }
  ...
  if (g.confidence === "High") {
    return addConfidenceFields({
      ...
      confidenceForceDebug: {
        ...
        calibrationMode: "v12_1_hitter_one_per_game_when_playable",
        viewMode,
        reason: "Hitter-prop candidate already cleared true High confidence gates."
      }
    });
  }

  const originalConfidence = g.confidence || "Low";
  const forceReason = "V12.1 one-per-game hitter-prop tracking: top playable hitter prop was promoted for collection because no true High cleared this game.";
  const modelSnapshot = g.modelSnapshot && typeof g.modelSnapshot === "object" ? {
    ...g.modelSnapshot,
    best: g.modelSnapshot.best && typeof g.modelSnapshot.best === "object" ? {
      ...g.modelSnapshot.best,
      originalConfidence,
      confidence: "High",
      forcedOnePerGameHitterProp: true,
      forcedOnePerGameReason: forceReason
    } : g.modelSnapshot.best
  } : g.modelSnapshot;

  const debug = g.debug && typeof g.debug === "object" ? {
    ...g.debug,
    selectedBest: g.debug.selectedBest && typeof g.debug.selectedBest === "object" ? {
      ...g.debug.selectedBest,
      originalConfidence,
      confidence: "High",
      forcedOnePerGameHitterProp: true,
      forcedOnePerGameReason: forceReason
  ...
  return addConfidenceFields({
    ...g,
    confidence: "High",
    confidenceScore: Math.max(Number(g.confidenceScore || 0), 72),
    pickScore: Math.max(Number(g.pickScore || 0), 72),
    recommendedTiming: g.recommendedTiming || "Track as top hitter-prop selection",
    recommendedStakeUnits: Number(g.recommendedStakeUnits || 0) > 0 ? g.recommendedStakeUnits : 0.5,
    reasons: safeArray(g.reasons).concat([forceReason]),
    analysisSummary: safeArray(g.analysisSummary).concat([forceReason]),
    analysisSummaryText: [g.analysisSummaryText, forceReason].filter(Boolean).join(" • "),
    trendTags: Array.from(new Set(safeArray(g.trendTags).concat(["forced_one_per_game_hitter_props", "v12_1_tracking_mode"]))),
    modelSnapshot,
    debug,
    confidenceForceDebug: {
      forceRan: true,
...
function selectOnePerGameView(games, viewMode) {
  return safeArray(games).map(g => {
    ...
    return addConfidenceFields({
      ...
      confidenceForceDebug: {
        forceRan: false,
        calibrationMode: "v12_1_true_confidence_for_non_hitter_views",
        viewMode,
        reason: "Non-hitter views remain true-confidence only. Hitter props use one-per-game tracking mode when a playable candidate exists."
      }
    });
  });
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      calibrationMode: "v11_phase1_data_trends_true_confidence",
      dashboardRevision: DASHBOARD_MODEL_REVISION,
      oddsDiagnostics
    });
  } catch (err) {
