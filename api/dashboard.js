...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const games = await Promise.all(limitedGames.map(async (game, index) => {
      ...
      const bestCandidate = chooseBestCandidate(filteredForView, scoredWithCorrelation, viewMode);
      const topPropCandidate = chooseTopPropCandidate(scoredWithCorrelation);
      const riskWarnings = buildRiskWarnings(componentScores, bestCandidate, scoredWithCorrelation, viewMode);
...
function buildPropCandidateFromOutcome(marketKey, outcome, lineupContext) {
  ...
  const officialLineupBonus = lineupContext.officialLineupAvailable ? 0.9 : -0.3;
...
function buildComponentScores({ moneylineData, totalData, lineupContext, parkFactor, probablePitchers }) {
  ...
  const lineupScore = lineupContext.officialLineupAvailable ? 0.55 : 0.15;
...
function scoreCandidates(context, candidates) {
  return (Array.isArray(candidates) ? candidates : []).map(candidate => {
    ...
    const confidence = determineConfidenceTier(finalScore, marketThresholds, candidateWithMeta, context);
...
function buildGamePayload({
  ...
  const firstFiveMoneyline = originalGame?.firstFiveMoneyline || "N/A";
  const firstFiveRunLine = originalGame?.firstFiveRunLine || "N/A";
  const firstFiveTotal = originalGame?.firstFiveTotal || "N/A";

  const boardMoneyline = viewMode === "f5" ? firstFiveMoneyline : moneylineData.display;
  const boardRunLine = viewMode === "f5" ? firstFiveRunLine : spreadData.display;
  const boardTotal = viewMode === "f5" ? firstFiveTotal : totalData.display;
  ...
  return {
    ...
    moneyline: boardMoneyline,
    runLine: boardRunLine,
    total: boardTotal,
    firstFiveMoneyline,
    firstFiveRunLine,
    firstFiveTotal,
    ...
    debug: {
      ...
      interpretation: {
        ...
        f5CandidatesGenerated: scoredCandidates.some(c => c.marketFamily === "f5")
      }
    }
  };
}
...
function getValueScore(candidate) {
  ...
  const fairProbBonus = candidate.fairProb != null && candidate.impliedProb != null
    ...
    : 0;
  return round2((edge * 1.10) + juiceBonus + fairProbBonus);
}

function getContextScore(candidate, context) {
  const lineupBonus = context.lineupContext.officialLineupAvailable ? 0.8 : -0.35;
...
function getMissingDataPenalty(componentScores) {
  let penalty = 0;
  if (componentScores?.liveFeedStatus?.bullpen !== "live") penalty += 0.30;
  if (componentScores?.liveFeedStatus?.scheduleTravel !== "live") penalty += 0.20;
  if (componentScores?.liveFeedStatus?.offenseVsHand === "placeholder") penalty += 0.20;
  if (componentScores?.liveFeedStatus?.defense === "placeholder") penalty += 0.10;
  return penalty;
}
...
function getMarketShapeBonus(candidate, context) {
  if (candidate.bestBetType === "Side") {
    return Math.abs(context.modelOutputs.sideComposite || 0) * 1.6;
  }
  if (candidate.bestBetType === "Total") {
    if (/^Under /.test(candidate.bestBet)) {
      const lowTotalBonus = context.totalData.point != null && context.totalData.point <= 8 ? 0.8 : 0.15;
      return (Math.abs(context.modelOutputs.totalComposite || 0) * 1.45) + lowTotalBonus;
    }
    return Math.abs(context.modelOutputs.totalComposite || 0) * 1.45;
  }
...
function getThresholdsForBetType(bestBetType, marketFamily, candidateType) {
  ...
  switch (key) {
    case "Side":
      return { high: 9.8, medium: 5.7, minEdgeHigh: 2.8, minEdgeMedium: 1.2 };
    case "Total":
      return { high: 9.7, medium: 5.6, minEdgeHigh: 2.6, minEdgeMedium: 1.1 };
    case "Run Line":
      return { high: 10.6, medium: 6.0, minEdgeHigh: 3.2, minEdgeMedium: 1.5 };
    case "F5 Side":
      return { high: 10.9, medium: 6.1, minEdgeHigh: 3.4, minEdgeMedium: 1.6 };
    case "F5 Total":
      return { high: 10.7, medium: 6.0, minEdgeHigh: 3.2, minEdgeMedium: 1.5 };
    case "F5 Run Line":
      return { high: 11.1, medium: 6.3, minEdgeHigh: 3.5, minEdgeMedium: 1.7 };
    case "prop":
      return /Home Runs/.test(candidateType || "")
        ? { high: 11.2, medium: 6.2, minEdgeHigh: 3.6, minEdgeMedium: 1.7 }
        : { high: 10.1, medium: 5.8, minEdgeHigh: 2.9, minEdgeMedium: 1.4 };
    default:
      return { high: 10.0, medium: 5.8, minEdgeHigh: 2.8, minEdgeMedium: 1.3 };
  }
}
...
function determineConfidenceTier(finalScore, thresholds, candidate, context) {
  ...
  if (
    ...
    completeness >= 45 &&
    (lineupOfficial || finalScore >= (thresholds.high + 0.7))
  ) {
    return "High";
  }
...
function recommendStakeUnits(finalScore, confidence) {
  if (confidence === "High") return finalScore >= 11 ? 1.25 : 1.0;
...
function getLineupSlotBonus(playerName, lineupContext) {
  ...
  if (slot == null) return lineupContext.officialLineupAvailable ? -0.5 : 0;
...
function buildPropReasonsV2(marketKey, outcome, lineupContext, edgePct, fairProb) {
  ...
  switch (marketKey) {
    ...
    case "batter_home_runs": reasons.push("Home run props stay eligible, but they need a stronger score to rate as high confidence."); break;
  }
  return reasons;
}
...
function buildRiskWarnings(componentScores, bestCandidate, candidates, viewMode) {
  ...
  if (viewMode === "f5" && !candidates.some(c => c.marketFamily === "f5")) warnings.push("F5 mode selected, but no distinct F5 candidates were generated from the current backend.");
  return warnings;
}
