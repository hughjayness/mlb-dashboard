...
function confidenceCalibrateCandidates(ranked, ctx) {
  // V9: same-game logic remains active, but High confidence is only allowed after rebalanced real-world-over-odds scoring.
  ...
  const arr = safeArray(ranked).map(c => {
    ...
    const reason = assigned === "High"
      ? `V9 balanced calibration: candidate cleared High threshold ${thresholds.high}, edge minimum ${thresholds.minEdgeHigh}, and same-game script review.`
      : assigned === "Medium"
        ? `V9 balanced calibration: candidate cleared Medium threshold ${thresholds.medium} but did not clear all High gates.`
        : `V9 balanced calibration: candidate did not clear High/Medium gates or was capped by same-game script review.`;
...
function forceViewConfidence(games, viewMode) {
  // V9 final lock: do not force High confidence. Enforce same-game caps, tighter High distribution, and no frontend inflation.
  ...
  if (!playable.length) {
    return arr.map(g => addConfidenceFields({
      ...
      confidenceForceDebug: {
        forceRan: true,
        calibrationMode: "v9_rebalanced_realworld_over_odds",
        viewMode,
        playableRows: 0,
