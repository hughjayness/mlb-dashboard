// api/dashboard.js
// Full replacement V12.6: Directional Edge Recalibration. Uses the 416-row / 376-graded Phase 1 window to preserve profitable directions and conditionally penalize repeated losing setups without blanket category suppression.
...
const DASHBOARD_MODEL_REVISION = {
  id: "v12-6-directional-edge-recalibration-2026-08-09",
  date: "2026-08-09",
  label: "V12.6 Directional Edge Recalibration: F5 Under conditional gating, home-side support, K Over restoration, TB Over skill gates, and context-first plus-money treatment"
};
...
// V12.5 retains the V12.5 hitter market funnel before confidence.
...
function v123HitterQualityGate(g) {
  // V12.5 keeps the name for compatibility, but the gate is now market-funneled.
...
function v126IsSideMarket(c) {
  return ["moneyline", "runline", "f5_moneyline", "f5_runline"].includes(c?.market);
}

function v126SideBaseballSupport(c, ctx, professional) {
  if (!v126IsSideMarket(c) || !c?.teamSide) return 0;
  const own = c.teamSide;

  const opp = opponentSide(own);
  const ownPitcher = Number(ctx?.realWorld?.pitchers?.[own]?.qualityScore || 0);
  const oppPitcher = Number(ctx?.realWorld?.pitchers?.[opp]?.qualityScore || 0);
  const ownOffense = teamOffenseComposite(ctx, own);
  const oppOffense = teamOffenseComposite(ctx, opp);
  const ownBullpen = Number(ctx?.realWorld?.teams?.[own]?.bullpenFreshnessScore || 0) + Number(getTeamContext(getSideTeamName(ctx, own)).bullpen || 0);
  const oppBullpen = Number(ctx?.realWorld?.teams?.[opp]?.bullpenFreshnessScore || 0) + Number(getTeamContext(getSideTeamName(ctx, opp)).bullpen || 0);
  const prof = Number(professional?.score || 0);
  let support = 0;
  if (ownPitcher - oppPitcher >= 0.06) support += 1;
  if (ownOffense - oppOffense >= 0.05) support += 1;
  if (ownBullpen - oppBullpen >= 0.04) support += 1;
  if (prof >= 0.08) support += 1;
  if (Number(c?.edgePct || 0) >= 1.0) support += 1;
  return support;
}

function v126F5MoneylineSupport(c, ctx, professional) {
  if (c?.market !== "f5_moneyline" || !c?.teamSide) return 0;
  const own = c.teamSide;
  const opp = opponentSide(own);
  const ownPitcher = Number(ctx?.realWorld?.pitchers?.[own]?.qualityScore || 0);
  const oppPitcher = Number(ctx?.realWorld?.pitchers?.[opp]?.qualityScore || 0);
  ...
  const oppOffense = teamOffenseComposite(ctx, opp);
  let support = 0;
  if (ownPitcher - oppPitcher >= 0.08) support += 1;
  if (ownOffense - oppOffense >= 0.04) support += 1;
  if (Number(professional?.score || 0) >= 0.08) support += 1;
  if (Number(c?.edgePct || 0) >= 1.15) support += 1;
  if (ctx?.sameGameScenario?.sideLean === own) support += 1;
  return support;
}

function v126TotalBasesHighSupport(c, professional) {
  if (c?.market !== "batter_total_bases" || candidateDirection(c) !== "over") return 99;
  const contact = Number(professional?.contactScore ?? c?.hitterContactScore ?? 0);
  const power = Number(professional?.powerScore ?? c?.hitterPowerScore ?? 0);
  const quality = Number(professional?.hitterQualityScore ?? c?.hitterQualityScore ?? 0);
  const spot = Number(professional?.lineupSpot ?? c?.lineupSpot ?? 99);
  const oppPitcher = Number(professional?.opponentPitcherQuality ?? c?.opponentPitcherQuality ?? 0);
  let support = 0;
  if (contact >= 0.08) support += 1;
  if (power >= 0.08) support += 1;
  if (quality >= 0.08) support += 1;
  if (Number.isFinite(spot) && spot <= 5) support += 1;
  if (oppPitcher <= 0.02) support += 1;
  return support;
}

function v126PostScenarioAdjustment(c, alignment, ctx) {
  let delta = 0;
  const tags = [];
  const notes = [];
  const scenario = ctx?.sameGameScenario || {};
  const prof = Number(c?.professionalRawScore || 0);

  // Current window: full-game totals are profitable in both directions. Reward alignment; do not blanket suppress Unders.
  if (c?.market === "total") {
    if (scenario.runLean === c.side) {
      delta += 0.14;
      tags.push(c.side === "over" ? "v126_game_over_script_aligned" : "v126_game_under_script_aligned");
    }
  }

  // Current window: F5 Over 16-11 (+10.3% ROI) vs F5 Under 1-5 (-65.7% ROI).
  // F5 Under remains available, but only gets punished when the actual early-game script disagrees.
  if (c?.market === "f5_total" && c?.side === "under") {
    if (scenario.runLean === "over") {
      delta -= 0.90;
      tags.push("v126_f5_under_conflicts_with_run_script");
      notes.push("V12.6: F5 Under conflicts with the same-game run script.");
    } else if (scenario.runLean === "under" && Number(c?.contextSupportCount || 0) >= 4) {
      delta += 0.12;
      tags.push("v126_f5_under_prevention_aligned");
    }
  }
  if (c?.market === "f5_total" && c?.side === "over" && scenario.runLean === "over") {
    delta += 0.20;
    tags.push("v126_preserve_f5_over_edge");
  }

  // Current window: home side bets 40-30-1 (+13.0% ROI) vs away 27-37 (-17.3% ROI).
  // This is conditional, not a blind home-team bonus.
  if (v126IsSideMarket(c)) {
    const support = v126SideBaseballSupport(c, ctx, c?.professionalContext || { score: prof });
    if (c.teamSide === "home" && support >= 3 && (scenario.sideLean === "home" || scenario.sideLean === "neutral")) {
      delta += scenario.sideLean === "home" ? 0.28 : 0.14;
      tags.push("v126_home_side_supported");
    }
    if (c.teamSide === "away" && support <= 2 && scenario.sideLean !== "away") {
      delta -= 0.32;
      tags.push("v126_away_side_needs_stronger_edge");
      notes.push("V12.6: away-side candidate lacks enough starter/offense/bullpen support to overcome the current directional leak.");
    }
  }

  // F5 ML sample is tiny (0-2-1); watchlist only. Require actual starter/early-side support before High promotion.
  if (c?.market === "f5_moneyline") {
    const support = v126F5MoneylineSupport(c, ctx, c?.professionalContext || { score: prof });
    if (support < 2) {
      delta -= 0.22;
      tags.push("v126_f5_ml_watchlist_weak_support");
    } else if (support >= 4 && scenario.sideLean === c.teamSide) {
      delta += 0.12;
      tags.push("v126_f5_ml_starter_script_support");
    }
  }

  return { delta: round2(delta), tags, notes };
}
...
function applyPriceDisciplineV122(c, score, tags) {
  ...
  // V12.6: plus-money was positive in the current window (+4.2% ROI), so remove the old +121..+160 blanket tax.
  // We still require baseball/context support; price itself never lifts the pick.
  if (odds > 0) {
    if (odds >= 161) { out -= 0.18; tags.push("long_plus_money_price_check_v126"); }
    else { tags.push("plus_money_context_first_v126"); }
    if (v126IsSideMarket(c) && Number(c?.edgePct || 0) < 0.5) {
      out -= 0.12;
      tags.push("plus_money_without_model_edge_v126");
    }
  }
  ...
  if (/Strikeouts/i.test(market) && direction === "over" && odds < -160) { out -= 0.20; tags.push("k_over_price_tax"); }
...
function applyMarketSpecificRulesV122(c, ctx, professional, score, tags) {
  ...
  if ((c.market === "total" || c.market === "f5_total") && direction === "under") {
    ...
    if (c.market === "f5_total") {
      // F5 Under remains a real directional leak; penalize only when prevention support is weak.
      if (support < 3) { out -= 0.78; tags.push("f5_under_lacks_prevention_context_v126"); }
      else if (support >= 4) { out += 0.08; tags.push("f5_under_prevention_supported_v126"); }
      if (line > 0 && line <= 4.0 && support < 4) { out -= 0.30; tags.push("f5_low_total_under_tax_v126"); }
      if (runEnv >= 0.05 && support < 4) { out -= 0.30; tags.push("f5_under_conflicts_with_run_environment_v126"); }
    } else {
      // Current window full-game Under is profitable; do not blanket suppress it.
      if (support < 2 && runEnv >= 0.08) { out -= 0.28; tags.push("game_under_needs_prevention_when_run_env_hot_v126"); }
      if (support >= 3) { out += 0.08; tags.push("game_under_prevention_supported_v126"); }
    }
  }
  ...
  if (c?.marketFamily === "prop" && c?.propSubType === "pitcher") {
    const isKOver = /Strikeouts/i.test(market) && direction === "over";
    if (!isKOver) {
      out -= 0.38;
      tags.push("pitcher_prop_watchlist_v126");
    }
    const noHigh = v123PitcherPropNoHighTag(c);
    if (noHigh) {
      tags.push(noHigh);
      out -= /Earned Runs/i.test(market) ? 2.60 : 1.75;
    }
  }
  ...
  if (/Total Bases/i.test(market) && direction === "over") {
    const supportCount = [
      ...
      oppPitcherQuality <= 0.02,
      ownOffense + runEnv >= 0.04
    ].filter(Boolean).length;
    if (!(hitterContact >= 0.08 && hitterPower >= 0.08)) { out -= 0.75; tags.push("tb_over_needs_contact_power_v126"); }
    if (supportCount < 3) { out -= 0.65; tags.push("tb_over_missing_skill_context_v126"); }
    if (supportCount < 4 && price != null && price < -130) { out -= 0.30; tags.push("tb_over_expensive_without_full_support"); }
    if (point >= 1.5 && supportCount < 4) { out -= 0.28; tags.push("tb_over_higher_line_needs_support_v126"); }
    if (supportCount >= 4 && hitterContact >= 0.10 && hitterPower >= 0.08) { out += 0.10; tags.push("tb_over_skill_context_supported_v126"); }
  }
  ...
  if (/Strikeouts/i.test(market) && direction === "over") {
    ...
    if (supportCount < 3) { out -= 0.90; tags.push("k_over_missing_support_v126"); }
    else if (supportCount === 3) { out -= 0.20; tags.push("k_over_moderate_support_v126"); }
    else if (supportCount >= 4) { out += 0.22; tags.push("v126_preserve_pitcher_k_over_edge"); }
  ...
  if (/Moneyline|Run Line/i.test(market)) {
    const enriched = { ...c, edgePct: Number(c?.edgePct || 0), professionalRawScore: profScore };
    const supportCount = v122SupportCount(enriched);
    const baseballSupport = v126SideBaseballSupport(enriched, ctx, professional);
    if (supportCount < 3 && baseballSupport < 2) { out -= 0.48; tags.push("side_lacks_context_edge_v126"); }
    if (c?.teamSide === "home" && baseballSupport >= 3) { out += 0.16; tags.push("home_side_baseball_support_v126"); }
    if (c?.teamSide === "away" && baseballSupport <= 1) { out -= 0.28; tags.push("away_side_weak_baseball_support_v126"); }
...
function hardRiskCapV122(tags) {
  const hard = [
    ...
    "pitcher_k_over_no_high_without_support_v126",
    "tb_over_no_high_without_contact_power_v126",
...
function scoreCandidate(c, ctx) {
  if (isInvalidCandidate(c)) {
    return addConfidenceFields({
      ...
      hybridNotes: ["V12.6 invalid-pick blocker removed this candidate from trust consideration."]
    });
  }
  ...
  const candidateWithEdge = { ...c, edgePct: edge };
  scored = applyPriceDisciplineV122(candidateWithEdge, scored, riskTags);
  scored = applyMarketSpecificRulesV122(candidateWithEdge, ctx, professional, scored, riskTags);
  ...
  if (c?.marketFamily === "prop" && c?.propSubType === "pitcher") {
    if (c.market === "pitcher_strikeouts" && c.side === "over") {
      if (tempSupport < 3) riskTags.push("pitcher_k_over_no_high_without_support_v126");
    } else if (tempSupport < 5) {
      riskTags.push("pitcher_prop_no_high_without_elite_alignment");
    }
  }
  if (c?.market === "batter_total_bases" && c?.side === "over" && v126TotalBasesHighSupport(c, professional) < 4) riskTags.push("tb_over_no_high_without_contact_power_v126");
...
function rankCandidates(candidates, ctx) {
  ...
  return prelim
    .map(c => {
      ...
      const directional = v126PostScenarioAdjustment(c, alignment, ctx);
      const adjustedScore = round2(Number(c.finalScore || 0) + Number(alignment.scriptAlignmentScore || 0) + Number(directional.delta || 0) - (alignment.scriptHardConflict ? 0.85 : 0));
      return addConfidenceFields({
        ...c,
        ...alignment,
        preScriptScore: c.finalScore,
        v126DirectionalAdjustment: Number(directional.delta || 0),
        riskTags: Array.from(new Set([...safeArray(c.riskTags), ...safeArray(directional.tags)])),
        hybridNotes: safeArray(c.hybridNotes).concat(safeArray(directional.notes)),
...
function candidateThresholds(c) {
  ...
  if (isPitcher) { high = 4.10; medium = 2.45; minEdgeHigh = 1.85; minEdgeMedium = 0.20; minCompletenessHigh = 74; }

  if (c.market === "total" && c.side === "under") {
    // Current window full-game Unders are profitable; treat direction symmetrically when prevention/script support is real.
    high += 0.05;
  }

  if (c.market === "f5_total" && c.side === "under") {
    // Keep a modest directional tax, with the main suppression now conditional on prevention + same-game script.
    high += 0.42;
    medium += 0.12;
    minEdgeHigh += 0.18;
  }
  ...
  if (c.market === "pitcher_strikeouts" && c.side === "over") {
    // Current window K Over is 16-12 (+14.1% ROI). Preserve only context-supported K Overs, but stop blanket over-tightening.
    high -= 0.18;
    minEdgeHigh = Math.max(1.35, minEdgeHigh - 0.35);
    minCompletenessHigh = Math.min(minCompletenessHigh, 72);
  }

  if (c.propSubType === "hitter" && c.side === "over" && (c.market === "batter_total_bases" || c.market === "batter_hits")) {
    high += c.market === "batter_total_bases" ? 0.24 : 0.18;
    medium += 0.10;
    minEdgeHigh += c.market === "batter_total_bases" ? 0.18 : 0.10;
  }
...
function v125IsDangerousHighBlockTag(tag) {
  return [
    "invalid_pick",
    "odds_missing_or_invalid",
    "weather_delay_pitcher_prop_risk",
...
function v125HasHardHighBlock(c) {
  if (!c || !c.bestBet || !hasValue(c.oddsPrice)) return true;
  if (c.scriptHardConflict || c.scriptConfidenceCap === "Low") return true;
  if (c.professionalConfidenceCap === "Low") return true;
  if (safeArray(c.riskTags).some(v125IsDangerousHighBlockTag)) return true;
  return false;
}

function v125IsSaferHighMarket(c) {
  const market = String(c?.market || "");
  const side = String(c?.side || "").toLowerCase();
  const price = toPrice(c?.oddsPrice);

  if (c?.marketFamily === "full") {
    if (market === "total" && side === "over") return true;
    if (market === "total" && side === "under") return Number(c?.contextSupportCount || 0) >= 3 && c?.scriptAlignmentStatus !== "soft_conflict";
    if (market === "moneyline") return c?.teamSide !== "away" || Number(c?.contextSupportCount || 0) >= 3 || c?.scriptAlignmentStatus === "aligned";
    if (market === "runline") return Number(c?.finalScore || 0) >= 3.20 && Number(c?.contextSupportCount || 0) >= 4;
  }

  if (c?.marketFamily === "f5") {
    if (market === "f5_total" && side === "over") return true;
    if (market === "f5_moneyline") return c?.scriptAlignmentStatus === "aligned" || Number(c?.contextSupportCount || 0) >= 4;
    if (market === "f5_runline") return Number(c?.finalScore || 0) >= 3.25 && Number(c?.contextSupportCount || 0) >= 4;
    if (market === "f5_total" && side === "under") return Number(c?.contextSupportCount || 0) >= 4 && c?.scriptAlignmentStatus === "aligned" && Number(c?.finalScore || 0) >= 3.10;
  }

  if (c?.marketFamily === "prop" && c?.propSubType === "hitter") {
    if (!SAFE_HITTER_HIGH_MARKETS.has(market)) return false;
    if (v124IsLongshotHitterMarket(c)) return false;
    if (price != null && (price < -190 || price > 170)) return false;
    if (side === "over" && Number.isFinite(Number(c?.lineupSpot)) && Number(c.lineupSpot) > 6) return false;
    return Number(c?.hitterQualityScore || 0) >= -0.05 || Number(c?.finalScore || 0) >= 3.05;
  }

  if (c?.marketFamily === "prop" && c?.propSubType === "pitcher") {
    return market === "pitcher_strikeouts" && side === "over" && Number(c?.contextSupportCount || 0) >= 4 && Number(c?.finalScore || 0) >= 3.25;
  }

  return false;
}

function v125PracticalHighFloor(c) {
  if (!v125IsSaferHighMarket(c)) return 99;
  if (c?.marketFamily === "full" && c?.market === "total" && c?.side === "over") return 2.55;
  if (c?.marketFamily === "full" && c?.market === "total" && c?.side === "under") return 2.68;
  if (c?.marketFamily === "f5" && c?.market === "f5_total" && c?.side === "over") return 2.52;
  if (c?.marketFamily === "full" && c?.market === "moneyline") return c?.teamSide === "home" ? 2.55 : 2.72;
  if (c?.marketFamily === "f5" && c?.market === "f5_moneyline") return 2.78;
  if (c?.marketFamily === "prop" && c?.propSubType === "hitter") return 2.70;
  if (c?.market === "pitcher_strikeouts" && c?.side === "over") return 3.22;
  return 3.05;
}

function v125CanPromoteToHigh(c) {
  if (v125HasHardHighBlock(c)) return false;
  if (!v125IsSaferHighMarket(c)) return false;
  if (c.trustFadePassDecision === "pass") return false;
  const score = Number(c?.finalScore || 0);
  const edge = Number(c?.edgePct || 0);
  const complete = Number(c?.dataCompleteness || 0);
  if (score < v125PracticalHighFloor(c)) return false;
  if (edge < -0.10) return false;
  if (complete < 52) return false;
  return true;
}

function v125ApplyPracticalHighRebalance(candidates, ctx) {
  let out = safeArray(candidates).slice().sort((a, b) => Number(b.finalScore || 0) - Number(a.finalScore || 0));
  const existingHighs = out.filter(c => c.confidence === "High").length;
  const candidatePool = out.filter(v125CanPromoteToHigh);

  // V12.6: retain practical slate-volume promotion, but use the new directional/context gates.
  // This is not a per-game force. It restores practical slate volume while retaining hard blocks on known leaks.
  const targetHighs = out.length >= 28 ? 4 : out.length >= 16 ? 3 : out.length >= 8 ? 2 : 1;
  const needed = Math.max(0, targetHighs - existingHighs);
  if (!needed || !candidatePool.length) return out;

  const promotedKeys = new Set(out.filter(c => c.confidence === "High").map(candidateKey));
  const promote = [];
  for (const c of candidatePool) {
    if (promotedKeys.has(candidateKey(c))) continue;
    promote.push(c);
    promotedKeys.add(candidateKey(c));
    if (promote.length >= needed) break;
  }
  if (!promote.length) return out;

  const promoteKeys = new Set(promote.map(candidateKey));
  out = out.map(c => {
    if (!promoteKeys.has(candidateKey(c))) return c;
    const reason = `V12.6 Directional Edge Recalibration: strict gates produced too little usable High volume, so this safer-market candidate was promoted after clearing hard-block, price, edge, and baseball-context floors.`;
    return setConfidence({
      ...c,
      practicalHighPromoted: true,
      decisionMode: c.decisionMode || "Trust Model",
      trendTags: Array.from(new Set([...safeArray(c.trendTags), "revision:v12-6-directional-edge-recalibration-2026-08-09", "practical_high_rebalance"])),
      riskTags: Array.from(new Set([...safeArray(c.riskTags), "practical_high_rebalance_v125"])),
      missedHighReasons: []
    }, "High", reason);
  });
  return out.sort((a, b) => Number(b.finalScore || 0) - Number(a.finalScore || 0));
}
...
function buildMissedHighReasons(c, thresholds) {
  ...
  if (safeArray(c.riskTags).length) missed.push(`V12.6 risk tags: ${safeArray(c.riskTags).join('; ')}`);
  return missed;
}
...
function assignBalancedConfidence(c) {
  ...
  const hardBlocked = v125HasHardHighBlock(c);
  const t = candidateThresholds(c);
  const score = Number(c.finalScore || 0);
  const edge = Number(c.edgePct || 0);
  const completeness = Number(c.dataCompleteness || 0);

  // V12.5: keep hard blocks for known leaks, but stop requiring a perfectly empty risk-tag list.
  // The previous V12.5 rule was too strict and produced no Highs for a week.
  if (!hardBlocked && score >= t.high && edge >= t.minEdgeHigh && completeness >= t.minCompletenessHigh && c.professionalConfidenceCap !== "Medium") return "High";

  // V12.6: safer-market practical High eligibility, expanded to profitable current-window directions while preserving conditional hard blocks.
  if (v125CanPromoteToHigh(c) && score >= v125PracticalHighFloor(c) + 0.28 && edge >= 0.35) return "High";

  if (score >= t.medium && edge >= t.minEdgeMedium) return "Medium";
  if (v125CanPromoteToHigh(c) && score >= v125PracticalHighFloor(c) - 0.18) return "Medium";
  return "Low";
}
...
function confidenceCalibrateCandidates(ranked, ctx) {
  const prime = safeArray(ranked).map(c => {
    ...
    const reason = assigned === "High"
      ? `V12.6 directional high-confidence calibration: value, baseball context, market-specific rules, no-bad-context gates, and same-game script cleared High threshold ${thresholds.high}.`
      : assigned === "Medium"
        ? `V12.6 directional high-confidence calibration: candidate cleared Medium threshold ${thresholds.medium} but did not clear every High gate.`
        : `V12.6 directional high-confidence calibration: candidate was capped or failed professional baseball, market-specific, invalid-pick, context, or historical gates.`;
  ...
  const calibrated = hybrid.map(c => {
    const thresholds = candidateThresholds(c);
    const assigned = assignBalancedConfidence(c);
    const mode = c.decisionMode || "Trust Model";
    const reason = assigned === "High"
      ? `V12.6 ${mode}: practical high-confidence rebalance cleared safer-market, price, baseball-context, no-hard-block, and historical trust/fade review.`
      : assigned === "Medium"
        ? `V12.6 ${mode}: usable candidate, but one or more professional-context, market-specific, risk, or historical gates kept it below High.`
        : `V12.6 ${mode}: candidate remained Low after practical high-confidence, professional-context, market-value, risk, and trust/fade/pass review.`;
    return setConfidence({
      ...c,
      highThreshold: thresholds.high,
      mediumThreshold: thresholds.medium,
      ...
      minCompletenessHigh: thresholds.minCompletenessHigh,
      missedHighReasons: buildMissedHighReasons(c, thresholds),
      trendTags: Array.from(new Set([...safeArray(c.trendTags), "revision:v12-6-directional-edge-recalibration-2026-08-09"]))
    }, assigned, reason);
  }).map(addConfidenceFields).sort((a, b) => Number(b.finalScore || 0) - Number(a.finalScore || 0));

  return v125ApplyPracticalHighRebalance(calibrated, ctx).map(addConfidenceFields).sort((a, b) => Number(b.finalScore || 0) - Number(a.finalScore || 0));
}
...
function buildGameResponse(event, ctx, ranked, filtered, viewMode) {
  ...
  const game = {
    ...
    marketAvailability: ctx.marketAvailability,
    debug: {
      confidenceGuarantee: "V12.6 does not blanket-promote every playable row. High requires emergency-recalibrated score/edge/completeness/script gates.",
      selectedBest: best ? {
...
function promoteHitterPropsOnePerGame(g, viewMode) {
  ...
  const base = {
    ...
    confidenceForceDebug: {
      ...
      reason: eligibleRestore
        ? "V12.6 restored this hitter prop to High only after lineup, edge, player-quality, and risk gates cleared."
        : playable
          ? "One hitter-prop candidate is displayed for the game, but V12.6 only promotes it when controlled quality gates clear."
  ...
  if (eligibleRestore) {
    return setConfidence({
      ...
      decisionMode: "V12.6 Controlled Hitter Restore",
      trustFadePassDecision: "trust",
      reasons: safeArray(base.reasons).concat(["V12.6 controlled hitter-prop restoration: confirmed/top-order hitter context and playable edge restored this candidate to High."])
    }, "High", "V12.6 controlled hitter-prop restoration cleared High gates.");
  }
  ...
  return safeArray(games).map(g => {
    ...
    return addConfidenceFields({
      ...
      confidenceForceDebug: {
        ...
        viewMode,
        reason: "V12.6 uses true confidence after professional-context, market-specific, and no-bad-context gates. No view is forcibly promoted to High."
      }
    });
  });
}
