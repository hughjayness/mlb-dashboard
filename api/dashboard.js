// api/dashboard.js
// Full replacement V6: odds ingestion diagnostics + same-game scenario arbitration.
  ...
  for (const cat of categories) {
    ...
    if (highTarget.scriptConfidenceCap === "Low") {
      Object.assign(highTarget, setConfidence(
        highTarget,
        "Medium",
        `V6 scenario arbitration: strongest ${cat.name} candidate had hard script conflict, capped at Medium. Scenario side=${scenario.sideLean}, run=${scenario.runLean}.`
      ));
    } else {
      Object.assign(highTarget, setConfidence(
        highTarget,
        "High",
        `V6 scenario arbitration: strongest ${cat.name} candidate supporting the winning script. Scenario side=${scenario.sideLean}, run=${scenario.runLean}.`
      ));
    }
    ...
    if (mediumTarget && mediumTarget.confidence !== "High") {
      Object.assign(mediumTarget, setConfidence(
        ...
        `V6 scenario arbitration: backup ${cat.name} candidate after script alignment review.`
      ));
    }

    for (const c of list) {
      if (c.scriptConfidenceCap === "Low" && c.confidence !== "High" && c.confidence !== "Medium") {
        Object.assign(c, setConfidence(c, "Low", `V6 same-game arbiter capped this candidate because ${c.conflictReason}`));
      }
    }
  }
...
function forceViewConfidence(games, viewMode) {
  // V6 final lock: confidence can be repaired only from the script-arbitrated selectedBest.
  ...
  if (!playable.length) {
    return arr.map(g => addConfidenceFields({
      ...
      confidenceForceDebug: {
        forceRan: true,
        calibrationMode: "v6_odds_ingestion_plus_scenario_arbitration",
        viewMode,
        playableRows: 0,
  ...
  for (const g of playable) {
    if (g.debug?.selectedBest?.scriptHardConflict || g.debug?.selectedBest?.scriptConfidenceCap === "Low") {
      Object.assign(g, setConfidence(g, "Low", "V6 final check: hard same-game conflict remains capped at Low."));
    }
  }
  ...
  if (highTarget) {
    Object.assign(highTarget, setConfidence(
      highTarget,
      "High",
      `V6 final check: strongest script-aligned ${viewMode} row confirmed High.`
    ));
  }
  ...
  if (mediumTarget && mediumTarget !== highTarget) {
    Object.assign(mediumTarget, setConfidence(
      mediumTarget,
      "Medium",
      `V6 final check: backup script-aligned ${viewMode} row confirmed Medium.`
    ));
  }
