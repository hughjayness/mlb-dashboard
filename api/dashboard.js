// api/dashboard.js
// Full replacement V2: hard guards against fake zero markets + guaranteed slate confidence for valid playable candidates.
...
async function fetchOddsList(apiKey, markets) {
  ...
  const urls = [
    `${ODDS_API_BASE}/sports/baseball_mlb/odds?regions=us,us2,uk,eu,au&oddsFormat=american&markets=${marketText}&apiKey=${encodeURIComponent(apiKey)}`,
    `${ODDS_API_BASE}/sports/baseball_mlb/odds?regions=us,us2&oddsFormat=american&markets=${marketText}&apiKey=${encodeURIComponent(apiKey)}`,
    `${ODDS_API_BASE}/sports/baseball_mlb/odds?regions=us&oddsFormat=american&markets=${marketText}&apiKey=${encodeURIComponent(apiKey)}`
  ];
  let best = [];
...
async function fetchEventMarkets(eventId, apiKey, markets) {
  ...
  const marketText = encodeURIComponent(markets.join(","));
  const urls = [
    `${ODDS_API_BASE}/sports/baseball_mlb/events/${encodeURIComponent(eventId)}/odds?regions=us,us2,uk,eu,au&oddsFormat=american&markets=${marketText}&apiKey=${encodeURIComponent(apiKey)}`,
    `${ODDS_API_BASE}/sports/baseball_mlb/events/${encodeURIComponent(eventId)}/odds?regions=us,us2&oddsFormat=american&markets=${marketText}&apiKey=${encodeURIComponent(apiKey)}`,
    `${ODDS_API_BASE}/sports/baseball_mlb/events/${encodeURIComponent(eventId)}/odds?regions=us&oddsFormat=american&markets=${marketText}&apiKey=${encodeURIComponent(apiKey)}`
  ];
  let merged = null;
  for (const url of urls) {
    try {
...
function candidateScript(c, ctx) {
  const script = { team: null, runEnv: null, pitcherTeam: null, pitcherSide: null, hitterSide: null };
  if (["moneyline", "runline", "f5_moneyline", "f5_runline"].includes(c.market)) script.team = c.teamSide || null;
  if (["total", "f5_total"].includes(c.market)) script.runEnv = c.side;
  if (c.propSubType === "hitter") { script.hitterSide = c.side; script.runEnv = c.side === "over" ? "over" : "under"; }
  if (c.propSubType === "pitcher") {
    ...
    script.runEnv = c.side === "over" ? "under" : "over";
  }
  return script;
}

function conflictSeverity(a, b, ctx) {
  const x = candidateScript(a, ctx);
  const y = candidateScript(b, ctx);
  let sev = 0;
  if (x.team && y.pitcherTeam && y.pitcherSide === "over" && x.team !== y.pitcherTeam) sev += 1.0;
  if (y.team && x.pitcherTeam && x.pitcherSide === "over" && y.team !== x.pitcherTeam) sev += 1.0;
  if (x.runEnv && y.runEnv && x.runEnv !== y.runEnv) sev += 0.55;
  if (x.team && y.team && x.team !== y.team) sev += 0.8;
  if (x.runEnv === "under" && y.hitterSide === "over") sev += 0.35;
  if (y.runEnv === "under" && x.hitterSide === "over") sev += 0.35;
  if (x.runEnv === "over" && y.pitcherSide === "over") sev += 0.18;
  if (y.runEnv === "over" && x.pitcherSide === "over") sev += 0.18;
  return round2(sev);
}
...
function rankCandidates(candidates, ctx) {
  ...
  const ranked = Array.from(deduped.values()).sort((a, b) => Number(b.finalScore || 0) - Number(a.finalScore || 0));
  const accepted = [];
  return ranked.map((c, i) => {
    let penalty = 0;
    let reason = "";
    for (const stronger of accepted.slice(0, 6)) {
      const sev = conflictSeverity(c, stronger, ctx);
      if (sev > penalty) { penalty = sev; reason = conflictReason(c, stronger, ctx); }
    }
    const adjusted = { ...c, slateRank: i + 1, conflictPenalty: penalty, conflictReason: reason, finalScore: round2(Number(c.finalScore || 0) - penalty), confidenceScore: confidencePct(Number(c.finalScore || 0) - penalty) };
    accepted.push(adjusted);
    return addConfidenceFields(adjusted);
  }).sort((a, b) => Number(b.finalScore || 0) - Number(a.finalScore || 0));
}

function confidenceCalibrateCandidates(ranked, ctx) {
  const arr = safeArray(ranked).map(c => ({ ...c, confidence: "Low" }));
  const categories = [
    { name: "full", test: c => c.marketFamily === "full" },
    { name: "f5", test: c => c.marketFamily === "f5" },
    { name: "hitter_props", test: c => c.marketFamily === "prop" && c.propSubType === "hitter" },
    { name: "pitcher_props", test: c => c.marketFamily === "prop" && c.propSubType === "pitcher" }
  ];
  for (const cat of categories) {
    const list = arr.filter(cat.test).filter(c => hasValue(c.oddsPrice)).sort((a, b) => Number(b.finalScore || 0) - Number(a.finalScore || 0));
    if (!list.length) continue;
    Object.assign(list[0], setConfidence(list[0], "High", `Slate calibration: top valid ${cat.name} candidate.`));
    const medium = list.find(c => c !== list[0] && conflictSeverity(c, list[0], ctx) < 1.0) || list[1];
    if (medium) Object.assign(medium, setConfidence(medium, "Medium", `Slate calibration: backup valid ${cat.name} candidate.`));
  }

  const highByGame = [];
  for (const c of arr.sort((a, b) => Number(b.finalScore || 0) - Number(a.finalScore || 0))) {
    if (c.confidence !== "High") continue;
    const hit = highByGame.find(h => conflictSeverity(c, h, ctx) >= 1.0);
    if (hit) Object.assign(c, setConfidence(c, "Medium", "Downgraded to avoid contradictory same-game High picks."));
    else highByGame.push(c);
  }
...
function forceViewConfidence(games, viewMode) {
  const playable = safeArray(games).filter(g => g && g.bestBet && g.bestBet !== "Pass" && hasValue(g.bestBetOdds));
  if (!playable.length) return games.map(addConfidenceFields);

  playable.sort((a, b) => Number(b.confidenceScore || 0) - Number(a.confidenceScore || 0));

  const top = playable[0];
  Object.assign(top, setConfidence(top, "High", `View guarantee: strongest valid ${viewMode} row set to High Confidence.`));

  const second = playable.find(g => g !== top && g.confidence !== "High");
  if (second) Object.assign(second, setConfidence(second, "Medium", `View guarantee: backup valid ${viewMode} row set to Medium Confidence.`));

  return games.map(addConfidenceFields);
}
...
async function buildGameContext(event, apiKey, scheduleGames, viewMode) {
  const detailed = await fetchEventMarkets(event.id, apiKey, ALL_MARKETS);
...
module.exports = async function handler(req, res) {
  ...
  try {
    const [oddsEvents, scheduleGames] = await Promise.all([
      fetchOddsList(apiKey, FEATURED_MARKETS).catch(() => []),
    ...
    for (const event of events) {
      try {
        const built = await buildGameContext(event, apiKey, scheduleGames, viewMode);
    ...
    return res.status(200).json({
      ...
      calibrationMode: "v2_view_guarantee_valid_markets_only"
    });
  } catch (err) {
