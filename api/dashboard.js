// api/dashboard.js
// Full replacement V5: same-game scenario arbitration; conflicts decide script winners without suppressing all picks.
...
function confidenceCalibrateCandidates(ranked, ctx) {
  ...
  const scenario = ctx.sameGameScenario || { sideLean: "neutral", runLean: "neutral" };
  ...
  for (const cat of categories) {
    const list = arr
      .filter(cat.test)
      .filter(c => c.bestBet && hasValue(c.oddsPrice))
      .sort((a, b) => Number(b.finalScore || 0) - Number(a.finalScore || 0));

    if (!list.length) continue;

    const highTarget =
      list.find(c => c.scriptAlignmentStatus === "aligned" && c.scriptConfidenceCap !== "Low") ||
      list.find(c => !c.scriptHardConflict && c.scriptConfidenceCap !== "Low") ||
      list[0];

    if (highTarget.scriptConfidenceCap === "Low") {
      Object.assign(highTarget, setConfidence(
        highTarget,
        "Medium",
        `V5 scenario arbitration: strongest ${cat.name} candidate had hard script conflict, capped at Medium. Scenario side=${scenario.sideLean}, run=${scenario.runLean}.`
      ));
    } else {
      Object.assign(highTarget, setConfidence(
        highTarget,
        "High",
        `V5 scenario arbitration: strongest ${cat.name} candidate supporting the winning script. Scenario side=${scenario.sideLean}, run=${scenario.runLean}.`
      ));
    }

    const mediumTarget = list.find(c =>
      c !== highTarget &&
      c.scriptConfidenceCap !== "Low" &&
      !c.scriptHardConflict
    ) || list.find(c => c !== highTarget);

    if (mediumTarget && mediumTarget.confidence !== "High") {
      Object.assign(mediumTarget, setConfidence(
        mediumTarget,
        mediumTarget.scriptConfidenceCap === "Low" ? "Low" : "Medium",
        `V5 scenario arbitration: backup ${cat.name} candidate after script alignment review.`
      ));
    }

    for (const c of list) {
      if (c.scriptConfidenceCap === "Low" && c.confidence !== "High" && c.confidence !== "Medium") {
        Object.assign(c, setConfidence(c, "Low", `V5 same-game arbiter capped this candidate because ${c.conflictReason}`));
      }
    }
  }
...
function forceViewConfidence(games, viewMode) {
  // V5 final lock: confidence can be repaired only from the script-arbitrated selectedBest.
  // It must not promote hard-conflicting rows above Low.
  const arr = safeArray(games);

  const playable = arr
    .filter(g => g && g.bestBet && g.bestBet !== "Pass" && g.bestBetType && g.bestBetType !== "Pass")
    .sort((a, b) => {
      const bHard = b.debug?.selectedBest?.scriptHardConflict ? 1 : 0;
      const aHard = a.debug?.selectedBest?.scriptHardConflict ? 1 : 0;
      if (bHard !== aHard) return aHard - bHard;
  ...
  if (!playable.length) {
    return arr.map(g => addConfidenceFields({
      ...
      confidenceForceDebug: {
        forceRan: true,
        calibrationMode: "v5_same_game_scenario_arbitration",
        viewMode,
        playableRows: 0,
  ...
  const nonConflicting = playable.filter(g => !g.debug?.selectedBest?.scriptHardConflict && g.debug?.selectedBest?.scriptConfidenceCap !== "Low");
  const highTarget = playable.find(g => g.confidence === "High") || nonConflicting[0] || null;
  const mediumTarget = playable.find(g => g !== highTarget && g.confidence === "Medium") || nonConflicting.find(g => g !== highTarget) || null;

  for (const g of playable) {
    if (g.debug?.selectedBest?.scriptHardConflict || g.debug?.selectedBest?.scriptConfidenceCap === "Low") {
      Object.assign(g, setConfidence(g, "Low", "V5 final check: hard same-game conflict remains capped at Low."));
    }
  }
  ...
  if (highTarget) {
    Object.assign(highTarget, setConfidence(
      highTarget,
      "High",
      `V5 final check: strongest script-aligned ${viewMode} row confirmed High.`
    ));
  }

  if (mediumTarget && mediumTarget !== highTarget) {
    Object.assign(mediumTarget, setConfidence(
      mediumTarget,
      "Medium",
      `V5 final check: backup script-aligned ${viewMode} row confirmed Medium.`
    ));
  }
  ...
  return arr.map(g => addConfidenceFields({
    ...
    confidenceForceDebug: {
      ...
      nonConflictingRows: nonConflicting.length,
      forcedHighBet: highTarget?.bestBet || null,
      forcedMediumBet: mediumTarget?.bestBet || null,
