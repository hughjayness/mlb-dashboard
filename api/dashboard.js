// api/dashboard.js
// Full replacement V12.2: Professional-context recalibration, invalid-pick blocking, context gates, overconfidence caps, and no forced High confidence.
...
const DASHBOARD_MODEL_REVISION = {
  id: "v12-2-professional-context-recalibration-2026-05-30",
  date: "2026-05-30",
  label: "V12.2 Professional Context Recalibration: invalid-pick blocker, no-bad-context gates, overconfidence cap, market-specific prop rules, targeted books"
};
...
function applyMarketSpecificRulesV122(c, ctx, professional, score, tags) {
  ...
  if (/Pitcher Outs/i.test(market) && direction === "over") {
    ...
    if (supportCount < 4) { out -= 0.90; tags.push("outs_over_workload_not_supported"); }
  ...
  if (/Strikeouts/i.test(market) && direction === "over") {
    ...
    if (supportCount < 4) { out -= 0.70; tags.push("k_over_missing_support"); }
...
function scoreCandidate(c, ctx) {
  if (isInvalidCandidate(c)) {
    return addConfidenceFields({
      ...
      hybridNotes: ["V12.2 invalid-pick blocker removed this candidate from trust consideration."]
    });
  }
  ...
  // V12.2: split betting value from baseball/professional context, then apply risk gates before confidence.
...
function buildMissedHighReasons(c, thresholds) {
  ...
  if (safeArray(c.riskTags).length) missed.push(`V12.2 risk tags: ${safeArray(c.riskTags).join('; ')}`);
  return missed;
}
...
function confidenceCalibrateCandidates(ranked, ctx) {
  const prime = safeArray(ranked).map(c => {
    ...
    const reason = assigned === "High"
      ? `V12.2 professional-context calibration: value, baseball context, market-specific rules, no-bad-context gates, and same-game script cleared High threshold ${thresholds.high}.`
      : assigned === "Medium"
        ? `V12.2 professional-context calibration: candidate cleared Medium threshold ${thresholds.medium} but did not clear every High gate.`
        : `V12.2 professional-context calibration: candidate was capped or failed professional baseball, market-specific, invalid-pick, context, or historical gates.`;
  ...
  return hybrid.map(c => {
    ...
    const reason = assigned === "High"
      ? `V12.2 ${mode}: professional baseball context, playable price, no-bad-context review, and historical trust/fade review cleared High.`
      : assigned === "Medium"
        ? `V12.2 ${mode}: usable candidate, but one or more professional-context, risk, market-specific, or historical gates kept it below High.`
        : `V12.2 ${mode}: candidate remained Low after professional-context, market-value, risk, and trust/fade/pass review.`;
    return setConfidence({
      ...c,
      highThreshold: thresholds.high,
      mediumThreshold: thresholds.medium,
...
function promoteHitterPropsOnePerGame(g, viewMode) {
  // V12.2: keep one displayed hitter candidate per game, but do not force it to High.
  // High Confidence must be earned after professional-context, price, and no-bad-context gates.
  if (!g || viewMode !== "hitter_props") return addConfidenceFields(g);

  const playable = g.bestBet && g.bestBet !== "Pass" && hasValue(g.bestBetOdds);
  return addConfidenceFields({
    ...g,
    trendTags: Array.from(new Set(safeArray(g.trendTags).concat(playable ? ["v12_2_hitter_best_candidate_displayed"] : ["v12_2_no_playable_hitter_prop"]))),
    confidenceForceDebug: {
      forceRan: false,
      calibrationMode: "v12_2_true_confidence_no_forced_high",
      viewMode,
      reason: playable
        ? "One hitter-prop candidate is displayed for the game, but V12.2 does not force it to High."
...
function selectOnePerGameView(games, viewMode) {
  return safeArray(games).map(g => {
    ...
    return addConfidenceFields({
      ...
      confidenceForceDebug: {
        forceRan: false,
        calibrationMode: "v12_2_true_confidence_all_views",
        viewMode,
        reason: "V12.2 uses true confidence after professional-context, market-specific, and no-bad-context gates. No view is forcibly promoted to High."
      }
    });
  });
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      calibrationMode: "v12_2_professional_context_recalibration",
      dashboardRevision: DASHBOARD_MODEL_REVISION,
      oddsDiagnostics
    });
  } catch (err) {
