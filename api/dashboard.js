// api/dashboard.js
// Full replacement V12.5: Practical High Confidence Rebalance. Keeps V12.5 protections but restores usable High volume through safer-market thresholds and slate-relative promotion.
...
const DASHBOARD_MODEL_REVISION = {
  id: "v12-5-practical-high-rebalance-2026-06-17",
  date: "2026-06-17",
  label: "V12.5 Practical High Confidence Rebalance: safer-market High restoration, slate-relative promotion, and V12.5 protection retention"
};
...
function scoreCandidate(c, ctx) {
  if (isInvalidCandidate(c)) {
    return addConfidenceFields({
      ...
      hybridNotes: ["V12.5 invalid-pick blocker removed this candidate from trust consideration."]
    });
  }
...
function v125IsSaferHighMarket(c) {
  ...
  const price = toPrice(c?.oddsPrice);
  const isDangerPlus = v123IsDangerPlusMoney(price);

  if (isDangerPlus && Number(c?.contextSupportCount || 0) < 5) return false;
  ...
  if (c?.marketFamily === "full") {
    ...
    if (market === "total" && side === "under") return Number(c?.contextSupportCount || 0) >= 5 && Number(c?.finalScore || 0) >= 3.55;
  }

  if (c?.marketFamily === "f5") {
    if (market === "f5_total" && side === "over") return true;
    if (market === "f5_moneyline") return true;
    if (market === "f5_runline") return Number(c?.finalScore || 0) >= 3.25 && Number(c?.contextSupportCount || 0) >= 4;
    if (market === "f5_total" && side === "under") return Number(c?.contextSupportCount || 0) >= 5 && Number(c?.finalScore || 0) >= 3.70;
  }
  ...
  if (c?.marketFamily === "prop" && c?.propSubType === "pitcher") {
    return market === "pitcher_strikeouts" && side === "over" && Number(c?.contextSupportCount || 0) >= 5 && Number(c?.finalScore || 0) >= 3.65;
  }
...
function v125PracticalHighFloor(c) {
  ...
  if (c?.marketFamily === "f5" && c?.market === "f5_total" && c?.side === "over") return 2.55;
  if (c?.marketFamily === "full" && c?.market === "moneyline") return 2.62;
  if (c?.marketFamily === "f5" && c?.market === "f5_moneyline") return 2.68;
...
function v125ApplyPracticalHighRebalance(candidates, ctx) {
  ...
  // V12.5: if the strict gate produced zero/slim High volume, promote the best safer-market candidates.
  ...
  out = out.map(c => {
    ...
    const reason = `V12.5 Practical High Rebalance: strict gates produced too little usable High volume, so this safer-market candidate was promoted after clearing hard-block, price, edge, and baseball-context floors.`;
    return setConfidence({
      ...c,
      practicalHighPromoted: true,
      decisionMode: c.decisionMode || "Trust Model",
      trendTags: Array.from(new Set([...safeArray(c.trendTags), "revision:v12-5-practical-high-rebalance-2026-06-17", "practical_high_rebalance"])),
...
function buildMissedHighReasons(c, thresholds) {
  ...
  if (safeArray(c.riskTags).length) missed.push(`V12.5 risk tags: ${safeArray(c.riskTags).join('; ')}`);
  return missed;
}
...
function assignBalancedConfidence(c) {
  ...
  // V12.5: safer-market practical High eligibility. This supports usable volume without reopening long-shot props/pitcher unders.
...
function confidenceCalibrateCandidates(ranked, ctx) {
  const prime = safeArray(ranked).map(c => {
    ...
    const reason = assigned === "High"
      ? `V12.5 practical high-confidence calibration: value, baseball context, market-specific rules, no-bad-context gates, and same-game script cleared High threshold ${thresholds.high}.`
      : assigned === "Medium"
        ? `V12.5 practical high-confidence calibration: candidate cleared Medium threshold ${thresholds.medium} but did not clear every High gate.`
        : `V12.5 practical high-confidence calibration: candidate was capped or failed professional baseball, market-specific, invalid-pick, context, or historical gates.`;
  ...
  const calibrated = hybrid.map(c => {
    ...
    const reason = assigned === "High"
      ? `V12.5 ${mode}: practical high-confidence rebalance cleared safer-market, price, baseball-context, no-hard-block, and historical trust/fade review.`
      : assigned === "Medium"
        ? `V12.5 ${mode}: usable candidate, but one or more professional-context, market-specific, risk, or historical gates kept it below High.`
        : `V12.5 ${mode}: candidate remained Low after practical high-confidence, professional-context, market-value, risk, and trust/fade/pass review.`;
    return setConfidence({
      ...c,
      highThreshold: thresholds.high,
      mediumThreshold: thresholds.medium,
      ...
      minCompletenessHigh: thresholds.minCompletenessHigh,
      missedHighReasons: buildMissedHighReasons(c, thresholds),
      trendTags: Array.from(new Set([...safeArray(c.trendTags), "revision:v12-5-practical-high-rebalance-2026-06-17"]))
    }, assigned, reason);
...
function buildGameResponse(event, ctx, ranked, filtered, viewMode) {
  ...
  const game = {
    ...
    marketAvailability: ctx.marketAvailability,
    debug: {
      confidenceGuarantee: "V12.5 does not blanket-promote every playable row. High requires emergency-recalibrated score/edge/completeness/script gates.",
      selectedBest: best ? {
...
function promoteHitterPropsOnePerGame(g, viewMode) {
  ...
  const base = {
    ...
    confidenceForceDebug: {
      ...
      reason: eligibleRestore
        ? "V12.5 restored this hitter prop to High only after lineup, edge, player-quality, and risk gates cleared."
        : playable
          ? "One hitter-prop candidate is displayed for the game, but V12.5 only promotes it when controlled quality gates clear."
  ...
  if (eligibleRestore) {
    return setConfidence({
      ...
      decisionMode: "V12.5 Controlled Hitter Restore",
      trustFadePassDecision: "trust",
      reasons: safeArray(base.reasons).concat(["V12.5 controlled hitter-prop restoration: confirmed/top-order hitter context and playable edge restored this candidate to High."])
    }, "High", "V12.5 controlled hitter-prop restoration cleared High gates.");
  }
  ...
  return safeArray(games).map(g => {
    ...
    return addConfidenceFields({
      ...
      confidenceForceDebug: {
        ...
        viewMode,
        reason: "V12.5 uses true confidence after professional-context, market-specific, and no-bad-context gates. No view is forcibly promoted to High."
      }
    });
  });
}
