...
function getThresholdsForCandidate(candidate) {
  ...
  switch (bucket) {
    ...
    case "hitter_tb_prop":
    case "hitter_hr_prop":
      return { high: 3.95, medium: 2.15, minEdgeHigh: 0.45, minEdgeMedium: 0.08, minCompletenessHigh: 24 };
    default:
...
function chooseBestCandidate(filteredCandidates, rankedCandidates, viewMode) {
  ...
  if (viewMode === "full") {
    const fallbackFull = ranked.find(c => c.marketFamily === "full");
    if (fallbackFull) return fallbackFull;
  }
  if (viewMode === "f5") {
    const fallbackF5 = ranked.find(c => c.marketFamily === "f5");
    if (fallbackF5) return fallbackF5;
  }
  if (viewMode === "pitcher_props") {
    const fallbackPitcher = ranked.find(c => c.marketFamily === "prop" && c.propSubType === "pitcher");
    if (fallbackPitcher) return fallbackPitcher;
  }
  if (viewMode === "hitter_props" || viewMode === "props") {
    const fallbackHitter = ranked.find(c => c.marketFamily === "prop" && (viewMode === "props" || c.propSubType === "hitter"));
    if (fallbackHitter) return fallbackHitter;
  }
