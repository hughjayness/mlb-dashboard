  ...
  try {
    ...
    const games = await Promise.all(
      limitedGames.map(async (game, index) => {
        ...
        let propResult = {
          topProp: null,
          status: shouldLoadProps
            ? "No supported BetMGM prop returned."
            : "Prop lookup skipped for current mode.",
          lineupMode: lineupContext.lineupMode,
          lineupSource: lineupContext.lineupSource
        };

        if (shouldLoadProps) {
          propResult = await fetchTopPropForEventCached(game.id, apiKey, lineupContext);
        }
    ...
    return res.status(200).json({
      ...
      notes: [
        ...
        shouldLoadProps
          ? "Props mode uses limited event-level lookups and short-lived caching to reduce rate-limit pressure."
          : "Prop lookups are skipped outside props mode to reduce rate-limit pressure.",
...
async function fetchTopPropForEventCached(eventId, apiKey, lineupContext) {
  const cacheKey = JSON.stringify({
    eventId,
    lineupMode: lineupContext && lineupContext.lineupMode ? lineupContext.lineupMode : "unknown",
    officialLineupAvailable: !!(lineupContext && lineupContext.officialLineupAvailable)
  });

  const cached = PROP_CACHE.get(cacheKey);
  if (cached && (Date.now() - cached.ts) < PROP_CACHE_TTL_MS) {
    return cached.value;
  }

  const value = await fetchTopPropForEvent(eventId, apiKey, lineupContext);
...
function buildRecommendation({
  ...
  if (viewMode === "props" && propResult && propResult.topProp && typeof propResult.topProp.modelProb === "number") {
