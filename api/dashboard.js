module.exports = async function handler(req, res) {
  ...
  try {
    const [oddsResponse, mlbLineups, teamStatsMap, umpireAssignments] = await Promise.all([
      fetch(baseUrl),
      fetchOfficialLineupsFromMLB(),
      fetchTeamSeasonStatsMap(),
      fetchUmpireAssignments()
    ]);
    ...
    const games = await Promise.all(
      limitedGames.map(async (game, index) => {
        ...
        const umpireContext = umpireAssignments[matchupKey(awayTeam, homeTeam)] || null;
    ...
    return res.status(200).json({
      ...
      notes: [
        ...
        "Umpire context is optional and only adjusts totals/F5 totals when a plate umpire assignment is available."
      ],
      viewMode,
      games
    });
...
const CACHE = {
  ...
  umpireAssignments: { expires: 0, data: {} }
};
...
async function fetchUmpireAssignments() {
  const now = Date.now();
  if (CACHE.umpireAssignments.expires > now && CACHE.umpireAssignments.data) return CACHE.umpireAssignments.data;

  const url = "https://www.actionnetwork.com/mlb/referee-assignments";

  try {
    const response = await fetch(url, { headers: { "user-agent": "Mozilla/5.0" } });
    if (!response.ok) return CACHE.umpireAssignments.data || {};

    const html = await response.text();
    const parsed = parseActionNetworkUmpires(html);

    CACHE.umpireAssignments = {
      expires: now + CACHE_TTL_MS.umpires,
      data: parsed
    };

    return parsed;
  } catch (error) {
    return CACHE.umpireAssignments.data || {};
  }
}

function parseActionNetworkUmpires(html) {
  ...
  const out = {};
  const abbrToTeam = buildAbbrToTeamMap();

  for (let i = 0; i < lines.length - 6; i++) {
    const awayTeam = abbrToTeam[lines[i]];
    const homeTeam = lines[i + 2] === "@" ? abbrToTeam[lines[i + 3]] : null;
    if (!awayTeam || !homeTeam) continue;

    const windowLines = lines.slice(i, Math.min(i + 30, lines.length));
    const officialIndex = windowLines.findIndex(v => v === "Official");
    if (officialIndex === -1) continue;

    const officialLine = windowLines[officialIndex + 3] || windowLines[officialIndex + 1] || "";
    const homeMlLine = windowLines[officialIndex + 4] || "";
    const underLine = windowLines[officialIndex + 5] || "";

    let umpireName = null;
    if (/^No Officials Data Yet$/i.test(officialLine)) umpireName = null;
    else if (/^[A-Z][a-z]+(?:\s+[A-Z][a-z]+)+$/.test(officialLine)) umpireName = officialLine;

    const homeMlPct = extractPctFromRecordLine(homeMlLine);
    const underPct = extractPctFromRecordLine(underLine);

    out[matchupKey(awayTeam, homeTeam)] = buildUmpireContext({
      umpireName,
      homeMlPct,
      underPct
    });
...
function buildAbbrToTeamMap() {
  const out = {};
  Object.keys(PARK_CONTEXT).forEach(team => {
    out[teamAbbrevHint(team)] = team;
  });
  out.OAK = "Athletics";
  out.ATH = "Athletics";
  out.WAS = "Washington Nationals";
  out.WSH = "Washington Nationals";
  out.ARI = "Arizona Diamondbacks";
  out.AZ = "Arizona Diamondbacks";
  out.SD = "San Diego Padres";
  out.SFG = "San Francisco Giants";
  out.SF = "San Francisco Giants";
  out.KC = "Kansas City Royals";
  out.TB = "Tampa Bay Rays";
  out.CHI = null;
  return out;
}

function extractPctFromRecordLine(line) {
  const m = String(line || "").match(/\((\d+(?:\.\d+)?)%\)/);
  return m ? Number(m[1]) : null;
}

function buildUmpireContext({ umpireName, homeMlPct, underPct }) {
  const totalLean = underPct == null ? 0 : clamp((underPct - 50) / 50, -0.35, 0.35);
  const sideLean = homeMlPct == null ? 0 : clamp((homeMlPct - 50) / 100, -0.15, 0.15);

  return {
    source: "Action Network referee assignments",
    plateUmpire: umpireName || null,
    homeMlPct,
    underPct,
    totalLean: round2(totalLean),
    sideLean: round2(sideLean),
    assigned: !!umpireName
  };
}
...
function buildComponentScores({
  ...
  const umpireTotalLean = clamp((toNumber(umpireContext && umpireContext.totalLean) || 0) * 1.0, -0.35, 0.35);
...
function buildDebugBlock({
  ...
  return {
    dataAvailability: {
      ...
      umpireAssigned: !!(umpireContext && umpireContext.assigned),
    ...
    rawInputs: {
      ...
      umpireContext: umpireContext || null,
    ...
    interpretation: {
      ...
      umpireZeroReason: explainZeroReason(componentScores.total.umpire, [!!(umpireContext && umpireContext.assigned)]),
