...
function forceViewConfidence(games, viewMode) {
  // V10B: every playable game row in the current board is promoted to High.
  // The per-game pick score remains visible for later back-testing and refinement.
  ...
  if (!playable.length) {
    return arr.map(g => addConfidenceFields({
      ...
      confidenceForceDebug: {
        forceRan: true,
        calibrationMode: "v10b_one_high_pick_per_game",
        viewMode,
        playableRows: 0,
  ...
  for (const g of playable) {
    Object.assign(g, setConfidence(
      g,
      "High",
      `V10B board rule: each playable game row is promoted to High so every game carries one top-ranked board pick. Score ${Number(g.pickScore ?? g.confidenceScore ?? 0)}.`
    ));
  }
  ...
  return arr.map(g => addConfidenceFields({
    ...
    confidenceForceDebug: {
      ...
      forcedHighRows: playable.map(x => x.bestBet),
      reason: "V10B promotes the single displayed playable pick for each game to High so the board always surfaces one High pick per game when a market exists.",
